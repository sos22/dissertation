...
\todo{Could consider explaining it in terms of supercompiling the
...
\item The first two productions show how to process
  ...
  \state{Copy} state, for instance, is handled by rule \circled{2} by
  performing the same copy operation and then advancing the crashing
  {\StateMachine} to its next state; an \state{If} state is handled by
  rule \circled{1} by testing the same expression and then advancing
  ...
  the other {\StateMachine}.  Some states, such as \state{Assert} or
  ...
  accessing states \state{Load} and \state{Store} are more
...
\item The next production, \circled{3}, shows how to handle non-local
  ...
  productions \circled{1} and \circled{2} are used so that local
...
\item Productions \circled{4}, \circled{5}, and \circled{6} show how
  to process terminal states.  \circled{4} and \circled{5} are
  ...
  crashing {\StateMachine} survives.  \circled{6} is only slightly
  ...
  the cross-product {\StateMachine} ends in the \state{Unreached}
  ...
  \state{Unreached} achieves this.

  Note that there is no symmetrical versions of productions
  \circled{4} and \circled{5}, and so the cross-product
...
\item The final three productions, \circled{7}, \circled{8}, and
  \circled{9}, show how to implement atomic blocks in the crashing
          {\StateMachine}.  Production \circled{7} allows the crashing
          {\StateMachine} to enter an atomic block, by discarding a
          \state{StartAtomic} state and setting the final,
          $\mathit{atomic}$, field of the \textsc{Configuration} to
          $\mathit{crashing}$.  Production \circled{8} shows how to
          exit one, by discarding a \state{EndAtomic} state and
          setting $\mathit{atomic}$ back to $\varnothing$.  Finally,
          production \circled{9} allows the crashing {\StateMachine}
          to perform non-local operations within its atomic
          block\footnote{Local operations in an atomic block are
            processed by production \circled{2}, the same as they
          ...
          Production \circled{7} is only valid if the atomic block is
          local to the crashing {\StateMachine}, which is true if
          every memory access reachable from $A$ before the first
          \state{EndAtomic} is local.  If that is not the case then
          the resulting 
\end{itemize}
...
\begin{figure}
  \begin{displaymath}
    ...
    \end{array}
  \end{displaymath}
  \caption{Cross-product algorithm productions which handle \state{If}
...
\begin{figure}
  ...
  \caption[Cross-product algorithm productions which handle local
    ...
    {\StateMachines}. Production \circled{$2_C$} is only valid when
    $a$ is local; production \circled{$2_I$} is only valid when $b$ is
...
\begin{figure}
  ...
  \caption[Cross-product algorithm productions which handle terminal
    ...
    {\StateMachines}.  Productions \circled{$6_C$} and \circled{$6_I$}
    are only valid if at least one of $i_a$ and $i_b$ are $\false$.
    Note that there are no productions \circled{$4_I$} and
    \circled{$5_I$}.}
...
\begin{figure}
  ...
  \caption[Cross-product algorithm productions which handle atomic
    ...
    In productions \circled{$7_C$} and \circled{$7_I$}, the atomic
