...
\todo{This section is generally a bit messy.}
...
which matches this initial non-terminal is \circled{A} with $n=3$,
which generates a new terminal $n = 3$, a new non-terminal
\graphNT{$3n+1$} = \graphNT{10}, and a single edge connecting them.
This new non-terminal matches production \circled{B} with $m = 10$,
and so that production is now applied.  It generates another terminal,
$m = 10$, and two new non-terminals, \graphNT{$\frac{m}{2}$} =
\graphNT{5} and \graphNT{$\frac{m}{2}-2$} = \graphNT{3}.  \graphNT{5}
is added to the graph in the obvious way.  \graphNT{3}, by contrast,
