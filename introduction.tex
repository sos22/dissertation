...
fix for the bug.  \smh{Very strong} The same analysis can also be used
...
beyond reproducing the bug which is to be fixed.\smh{Algorithms+modes
  seems odd.}

One the other hand, the class of bugs to be targeted is somewhat
limited\smh{Why?}: two threads interacting, with one modifying a shared
...
limitation, but still allows some realistic bugs to be
analysed.\smh{Why is this important (or not)?  i.e. can we reduce
  other bugs to this or extend the technique?}
...
\todo{To be honest, I'm not really convinced by the abandonware story
  ...
  something I really want to emphasise.}\smh{Agree}
...
\begin{tikzpicture}
  ...
  \node [block, below = of dynamic] (model) {Program model};
  \node [block, below = of model] (static) {Static analysis};
...
mostly\smh{c/f RX?} consist of inserting additional delays, and hence
...
\StateMachine\editorial{Desperately need a better name for
  these.}\smh{``sketch''? (What's in it?  What does it look like?}
...
\item
  ...
  program itself and library functions\editorial{But only in certain
    modes: I was a bit lazy in the bug-finding mode and restricted
    things to the main binary.  It's just an artifact of the
    implementation, and could be fixed without too much pain, but not
...
occur\smh{Do you mean this?  ``Constructing an \emph{analysis} to make
  the \emph{hb graph} more likely to \emph{occur}. Huh?}.  The
...
\item Only \emph{concurrency} bugs are considered.  This means that
  ...
  this might be unsound.}.  \todo{I want to say something about still
  being able to enforce arbitrary HB-graphs, rather than the very
  simple ones you get with critical sections, but can't quite get the
  phrasing right.}
\item The only communication allowed is from the \emph{write} thread
  ...
  the two threads being examined.\smh{Is this a big restriction?  Can
    we not frame many bugs into this shape?}
\item The read thread must crash \emph{quickly}: within the
  ...
  \verb|abort| or \verb|__assert_fail|.\smh{Can spin this around --
    you require a bug has manifested detector.  Some simple ones are
    segv() or assert() or panic() but something more complex would be
    ok too, e.g TESLA.}
\end{itemize}
