...
\item It assumes that the processor implements a strongly-ordered
  ...
  reordered.  The behaviour of modern processors is far more complex
  than this~\cite{Sarkar2009}, and so {\technique} cannot precisely
  model any widely-used processor.  On the other hand, the current
  most widely-used architecture, AMD64\needCite{}, implements a
  reasonably strong memory ordering where this is not a completely
  unreasonable approximation, and so this limitation is not completely
  crippling.
