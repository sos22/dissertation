...
program accesses memory when it is operating normally, using
\begin{wrapfigure}{r}{5.4cm}
  ...
  \vspace{-24pt}
\end{wrapfigure}
primarily dynamic analysis (\autoref{sect:program_model}).  The
...
description of the program's behaviour, as the effects of compiler
optimisations are often important when investigating concurrency
errors, but, on the other hand, source code provides far more useful
...
I refer to bugs which satisfy these constraints as \glspl{simple
  atomicity violation}.  This clearly does not include every possible
type of concurrent bug (it does not, for instance, include any but the
most trivial deadlock bugs, and complicated memory corruption bugs are
difficult to handle), but does include some interesting ones.
