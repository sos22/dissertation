...
\item It'd be nice to have something showing the effectiveness of the
  simplification bits.
...
rather large, but is probably close to {\technique}'s worst
...
\begin{table}
  ...
  \caption{Performance of some variants of the fix, in millions of
    ...
    completed $352.5 \pm_p 0.2$ million iterations per second, when it
...
\begin{figure}
  ...
  \caption{Time taken by the per-\gls{crashingthread} phases with
    {\StateMachine} simplification disabled.}
...
Figure~\ref{fig:perf:alpha:bpm:unopt}.  Note that the means were
calculated discarding timeouts, whereas the quantiles were calculated
treating timeouts as having taken precisely five minutes.
...
\todo{This is going to need re-writing once I get the results of the
  current batch of experiments.}
