...
\begin{itemize}
  ...
  \item The \bugname{multi\_variable} test,
    ...
    this kind of bug.
  \item \todo{urk} The final test, \bugname{double\_free}, is intended
...
\begin{sanefig}
  \input{eval/artificial_bugs/cdf1.tex}
...
\begin{sanetab}
  \begin{tabbular}{|l|l|l|l|}
    \hline
                           & \multicolumn{3}{c|}{Time to run main loop} \\
    \cline{2-4}
  ...
  \caption{Time taken to run a single iteration of the main loop of
    each test, in microseconds, with and without a fix applied.  This
    experiment was structured as eleven batches, with each
    configuration tested once in each batch and the results of the
    first batch discarded.  A configuration was tested by running it
    for ten seconds, restarting whenever the test program crashed, and
    ...
    and present summary statistics for that distribution.  The final
    column gives the differences between the fixed and unfixed
    configurations expressed as $x \pm y$, where $x$ is the mean
    difference and $y$ the standard deviation in $x$, both calculated
    from the other data in the table.  \todo{Need more data.  Also,
      make sure it's after figure 7.2.  And maybe try to trim the
      caption.}}
...
\begin{sanetab}
  \begin{tabbular}{|p{2.72cm}|l|l|l|}
    ...
    \hline    
  \end{tabbular}
  \caption{Time taken building the various \glspl{bugenforcer}. \todo{Need more data}}
...
\begin{sanetab}
  ...
  \caption{Reproduction counts for the different bugs and
    ...
    \bugname{thunderbird} ones ten. \todo{Need more data}}
...
of 30ms and a mean of 1500ms\editorial{No it doesn't!}.  This is
...
The per-crashing instruction generates a large number of pairs of
crashing {\StateMachine} and interfering \gls{cfg}, and these must
then be analysed; the time taken to do so is illustrated in
\autoref{fig:eval:how:per_interfering}, in the same style as
\autoref{fig:eval:how:per_crashing_times}.
\autoref{tab:eval:how:failures_per_interfering} details the failures
...
\begin{sanefig}
  ...
  \caption{Time taken by per-\gls{interferingthread} analysis steps,
    as distributions over the 23559 interfering \glspl{cfg} generated
...
time taken is only 17ms\editorial{Check numbers}.  \editorial{odd
  linkage} It also increased the number of
...
\todo{I don't think it's a coincidence that 17ms is very close to the
...
{\technique}'s utility\editorial{ugg}.
...
\begin{sanefig}
  ...
  \caption{Time taken to analyse the \bugname{complex\_hb}$_N$ test,
    ...
    Crosses and bars give the mean and standard deviation of mean of
    these samples.  The solid line shows a least-squares exponential
  ...
  \subfigure[][Crashing thread]{
    ...
    \begin{tabular}{ll}
      ...
      &\hspace{-2.3mm}\begin{tabular}{lll}
         assert(~(&\hspace{-2.5mm}$\texttt{slots}[\texttt{loadIdx}_1]$ &\hspace{-3mm} + \\
         &\hspace{-2.5mm}$\texttt{slots}[\texttt{loadIdx}_2]$ &\hspace{-3mm} + \\
         & \vdots \\
         &\hspace{-2.5mm}$\texttt{slots}[\texttt{loadIdx}_L]$ &\hspace{-4.5mm} ) \\
         &\multicolumn{2}{l}{\hspace{1em}!= $L{\times}\texttt{NR\_PTRS} + 1$);}\\
       \end{tabular}\\
      \multicolumn{2}{l}{\}}
    \end{tabular}
    }
...
\subsection{A bug which lacks the \glsentrytext{w-isolation} property (i\_isolation)}
