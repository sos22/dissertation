...
\begin{table}
  \begin{tabular}{lll}
    ...
    Time to build \gls{crashingthread} {\StateMachine}, seconds & $0.5 \pm 0.1$* & $0.6 \pm 0.1$* \\
    ...
    Time to build \glspl{interferingthread} {\StateMachines}, seconds & $0.11 \pm 0.02$* & $0.12 \pm 0.01$* \\
    Timeouts in symbolic execution & 35 & 281 \\
    \hspace{5mm}As percentage of \glspl{interferingthread} & 0.9\% & 7.6\% \\
    Symbolic execution time per \glspl{interferingthread}, seconds & $0.96 \pm 0.07$* & $1.31 \pm 0.08$* \\
  ...
  \end{tabular}
  \caption{Effects of the \gls{w-isolation} assumption.
    *: mean and standard deviation of mean for all runs which did not time out.
    $\dagger$: mean and standard deviation of mean}
...
\todo{This'll be pretty basic: run the analysis with input from the
  dynamic analysis, then run without, and see how much longer it takes
  and how many more false positives and timeouts we get.}
...
PIN\needCite{} might provide a useful speed-up.
...
\todo{This'll just be some pretty noddy profiling things showing how
  the analysis time breaks down into the different phases for the
  ``normal'' configuration (i.e. how much in copy prop, how much in
