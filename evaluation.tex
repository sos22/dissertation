...
\todo{All of my bootstraps are pure non-parametric bootstraps.  I
  should arguably be mixing in some Gaussian noise as well, just for
  sanity, but that means re-running all the statistical stuff yet
  again.}
...
\begin{sanefig}
  ...
  \caption{Distributions of time taken by the per-crashing instruction
    steps of the analysis. \todo{No error indicator on failure or skip rates.}}
...
\begin{sanefig}
  ...
  \caption{Time taken by per-\gls{interferingthread} analysis steps, in seconds,
    ...
    {\technique} has identified the interfering {\StateMachine}. \todo{Explain secondary mode running cross-product.}}
...
\begin{sanefig}
  ...
  \caption{Time taken by per-\gls{interferingthread} analysis steps,
    in seconds, with the \gls{ic-atomic} steps disabled. \todo{Get rid
      of the redundant abscissae.}}
...
\todo{Not entirely convinced this is the right place for this.}
...
\begin{sanefig}
  ...
  \caption{Dynamic aliasing coverage against time for MySQL, using
    ...
    where the program was restarted.  \todo{No error bars on this one.
      I'm not sure how you'd go about adding any, either.}}
...
\begin{sanefig}
  ...
  \caption{Reproduction times with and without an enforcer loaded, for
    ...
    with 1000 replicates. \todo{Fill in boxes with white.}}
...
number of such registers available in most processors).  \todo{Might
  want a bit more here.}
...
exponential regression on the data for $7 \leq N \leq 20$\footnote{The
  ...
  boundaries of this region.}\!\hspace{-.1ex}\!, extrapolated over the complete range of
...
\begin{sanefig}
  ...
  \caption{Memory and time used to analyse the
    ...
    if the heap size exceeded 2GB. \todo{Regenerate with all the
      contours.}}
...
\begin{sanefig}
  ...
  \caption{Memory and time used to analyse the
    ...
    \autoref{fig:eval:why:complex_aliasing:result1}. \todo{Regenerate.}}
...
\autoref{fig:eval:why:complex_aliasing:result2}.  As expected,
        ...
        far larger values of $L$\footnote{The largest successful value
          ...
          (S+1).1$.}\hspace{-1ex}.  This behaviour is clearly more useful than
...
\todo{Not brilliantly written, it has to be said.}  \todo{Currently
  rerunning these experiments with a bigger timeout; update once I
  have more results.}
...
finished, so as to avoid certain artifacts caused by contending to
allocate memory at high values of \gls{alpha}.  Steps which timed out
were not re-run, as each thread had a dedicated core and so there was
far less contention for CPU time.
