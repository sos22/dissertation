...
measures\footnote{Using a bootstrap method when
necessary.}\editorial{Should make it clear that these are intervals
for the \emph{means} rather than for the data.} and use these to
calculate the minimum and maximum overhead estimates where both
performance metrics are within their confidence intervals.  This
...
describing it.}  \todo{Alternative would be to use a Hwang bootstrap
to estimate confidence intervals on the ratio directly, but there have
been a bunch of papers recently saying that that gives you intervals
which are too tight when its preconditions don't fit, and I don't know
enough stats to check whether they do here.}
...
\todo{\emph{Should} do all of that stuff.  Currently just report the ratio of means.}
