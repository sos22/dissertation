...
\todo{Should use a 1000 replicate bootstrap for this, really, not a
  100 replicate one.} This is further illustrated in
...
time taken was unchanged\footnote{In fact, the average time taken
  actually \emph{increased} slightly, from $0.52 \in [0.46,
    0.60]_{100}^{27338}$ to $0.59 \in [0.50, 0.68]_{100}^{27432}$, but
  that was not statistically significant.}.  Disabling \gls{ic-atomic}
processing also increased the number of \glspl{verificationcondition}
generated, from 3870 to 6613.  These extra conditions are all, by
construction, false positives, and all would require run-time
verification.  \todo{Not sure how much that added, really.}
...
\todo{Not happy with the writing here.  Also, it's not all that clear
  what I'm changing in the different experiments.}
...
incorporate them into the {\StateMachine} analysis.  Some uses of the
model are necessary for {\technique} to function at all (without the
dynamic aliasing model, {\technique} cannot derive $\beta$ and $c2i$,
so cannot build the interfering {\StateMachines}; without the static
analysis, it cannot find instruction predecessors, and so cannot build
the crashing \gls{cfg}), but others are used merely to improve
performance (the dynamic analysis provides hints to the alias
analysis, for instance, and the static analysis can identify points in
the program where particular registers are guaranteed to have
particular values).  To evaluate the effectiveness of these analyses,
I re-analysed the 10,000 instructions selected in
\autoref{sect:eval:how_does_it_work} with these optional components
disabled.  The results are shown in
\autoref{tab:eval:why:program_model}.

\begin{sanetab}
  \begin{tabbular}{|l|l|l|l|}
    \hline
    & Full model & No dynamic analysis & No static analysis \\
    \hline
    Time to process crashing instruction & $x \pm_\mu^n y$ \\
    Interfering \glspl{cfg} per crashing instruction* & $x \in [a,b]^n_{1000}$ \\
    Interfering \glspl{cfg} generating \glspl{verificationcondition}$\dagger$ & $x \in [a,b]^n_\infty$\% \\
...
useful but not critical, giving modest reductions in analysis time, in
the failure rate, and in the number of \glspl{verificationcondition}
generated.  {\Technique} remains useful, albeit slower and less
effective, when the non-essentially information collected by the
static analysis is ignored.
