...
\item Re-do pbzip2 and collect numbers.  \todo{Run on ise overnight.}
...
\item The convergence experiments for the dynamic analysis phase.
  Need to do some other test, rather than just the one mysql test.
\end{itemize}
...
\begin{figure}
  ...
  \caption{\Gls{cfg} and {\STateMachine} for the crashing thread
...
analysis will ask\editorial{ick} the \gls{programmodel} what stores
...
analyse synchronisation within the analysis window \gls{alpha}.  The
analysis will therefore be aware of the synchronisation in the
...
\todo{Should probably say more about this e.g. what \gls{alpha} you'd
  need to analyse it and how far I was actually able to push it.}
...
\todo{This is a bit sad; basically a negative result.  Oh well.}  I
...
\todo{Whoops: only recorded which instructions timed out for the last
  run.  Re-do that.}


\todo{How long do the enforcers take to run?}  \todo{Timeout
  settings?}  \todo{Version of pbzip2?}  \todo{Compile settings?}
\todo{Value of $\alpha$?}
...
\todo{I want to say something about how good/bad the dynamic analysis
  is.  Things to measure:}
...
behaviour, and how much does it slow the program down while it is
being analysed?  I investigate the first concern first.

For this experiment, I monitored mysql while running the
rpl\_change\_master test out of its test suite under the dynamic
analysis and arrange to take a snapshot of the aliasing table every
1,000,000 basic blocks.  This corresponded to running for roughly
100ms between snapshots.  I treated the aliasing table obtained after
running for quarter of an hour as a ground truth and calculated the
proportion of ground truth aliasing pairs which were present in each
snapshot.  The results are shown in
Figure~\ref{fig:eval:dyn_convergence}.  Note that this test involves
starting and restarting mysql several times; the vertical dashed bars
show where it restarted.

This graph shows that the dynamic analysis achieved reasonable
coverage for this test after five minutes, and essentially complete
coverage after ten.

\begin{figure}
  \input{eval/dyn_convergence/dyn_convergence.tex}
  \caption{Dynamic convergence}
  \label{fig:eval:dyn_convergence}
\end{figure}


\todo{Repeat for some of the other programs.}
