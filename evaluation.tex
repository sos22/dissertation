...
in Figure~\ref{fig:eval:time_breakdown:convert_to_enforcer}\footnote{I
  ...
  results.}.  Not all of the \glspl{verificationcondition} were
converted successfully:

\begin{itemize}
\item 49 of the \glspl{verificationcondition}, or about 1\%, reached
  the one-minute timeout while slicing the \gls{verificationcondition}
  by happens-before graph.  As discussed in
  \autoref{sect:enforce:slice_hb_graph}, the slicing algorithm has an
  extremely bad worst-case complexity but very rarely actually
  encounters it, and so this is not particularly surprising.
\item 11 \glspl{verificationcondition}, or about 0.2\%, ran out of
  memory during the side-condition placement phase, on a system with
  8GiB of memory.  Like the slicing algorithm, the placement algorithm
  has a very bad worst case which rarely happens in practice, and so
  again this is not entirely surprising\footnote{This rate probably
    overstates the rate which would be observed by a real
    implementation of {\technique}: {\implementation} manages most of
    the relevant data structures using a garbage collector, but, for
    implementation reasons, the GC is disabled during side-condition
    placement.  This is likely to have significantly increased
    {\technique}'s memory usage in a way which is not completely
    reflective of the algorithm's actual requirements.}.
\item Three \glspl{verificationcondition} reached the one-minute
  timeout while computing a patch strategy (see
  \autoref{sect:enforce:gain_control}).  \todo{Explain why it failed.}
\end{itemize}
