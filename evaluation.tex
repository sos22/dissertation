...
without the fix,\kern-.5pt\fnote{Due to implementation limitations, it
  is not possible to load a {\technique} fix and a
  {\technique} \gls{bugenforcer} into the same program.} but
...
\hline
\end{tabbular}
\caption{Effectiveness of {\technique} at finding previously
...
the {\technique} aliasing table during the dynamic analysis
...
\item \subcrash{5} -- invoke phase \subinterfering{} so as to process
...
sub-phase \subenf{2}.  This was due to the incompleteness of the SMT
...
failures, and causes {\technique} to generate a slightly smaller
...
\begin{sanetab}
  ...
  \begin{tabbular}{|>{\HangingRaggedRight} p{6.2cm}| >{\RaggedRight \hspace{-1mm}}p{2.55cm}| >{\RaggedRight}p{2.6cm} | >{\RaggedRight}p{2.6cm}|}
    ...
    \hline
  \end{tabbular}
  \caption{Effect of the \glsentrytext{programmodel} on analysis
    effectiveness.  For the static disabled column, {\technique}
    was configure to only use information from the static alias
    analysis when deriving the static crashing \glsentrytext{cfg}; for
...
\noindent
This section explores {\technique}'s behaviour as the complexity
...
enforced~\cite{Musuvathi2008}, and {\technique} was able to
...
\gls{interferingthread}, common to both the \bugname{easy} and
\bugname{hard} variants, consists of a single store.  The difference
