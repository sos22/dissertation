...
\begin{figure}
  ...
  \subfigure[][CI atomic]{
    \begin{tabular}{ll}
      ...
      \tt 11 & \tt v1 = 5;\\
      \\
      \multicolumn{2}{c}{$\smLoad{\texttt{global1}} = \smLoad{\texttt{global2}}$}\\
    \end{tabular}
  }
  \hfill
  \subfigure[][IC atomic]{
    \begin{tabular}{ll}
      ...
      \tt 5 & \tt assert(v1 == v2);\\
      \\
      \multicolumn{2}{c}{$\smLoad{\texttt{global2}} = 5$}\\
    \end{tabular}
  }
  \hfill
  \vspace{12pt}
...
\begin{figure}
  \centerline{\input{eval/complex_hb/complex_hb_build_summaries.tex}}
...
\todo{Look at how long the bug takes to reproduce.}

\subsection{A program with more complicated aliasing behaviour}

\begin{figure}
  \texttt{
    \begin{tabular}{lll}
      \multicolumn{3}{l}{while (1) \{}\\
      & \multicolumn{2}{l}{for $0 \leq \texttt{i} < \texttt{nr\_stores}$}\\
      & & $\texttt{store\_idx}_{\texttt{i}}$ = random() \% 100;\\
      & \multicolumn{2}{l}{for $0 \leq \texttt{i} < \texttt{nr\_loads}$}\\
      & & $\texttt{load\_idx}_{\texttt{i}}$ = random() \% 100;\\
      & \multicolumn{2}{l}{STOP\_ANALYSIS();}\\
      & \multicolumn{2}{l}{slots[$\texttt{store\_idx}_0$] = 1;}\\
      & \multicolumn{2}{l}{slots[$\texttt{store\_idx}_1$] = 1;}\\
      & \vdots \\
      & \multicolumn{2}{l}{slots[$\texttt{store\_idx}_{\texttt{nr\_stores}}$] = 1;}\\
      & \multicolumn{2}{l}{assert(slots[$\texttt{load\_idx}_0$] != 2 \&\&} \\
      & &       slots[$\texttt{load\_idx}_1$] != 2 \&\& \\
      & &       \vdots\\
      & &       slots[$\texttt{load\_idx}_{\texttt{nr\_loads}}$] != 2);\\
      & \multicolumn{2}{l}{STOP\_ANALYSIS();}\\
      \}\\
    \end{tabular}
  }
  \caption{The test program for complicated aliasing tests.
    \texttt{slots} is a global array of 100 integers.}
  \label{fig:eval:complex_alias}
\end{figure}

This test program is intended to investigate {\implementation}'s
behaviour when confronted with more complicated aliasing problems.
The test program, shown in \autoref{fig:eval:complex_alias} consists
of a single thread which manipulates a global array, first setting
some randomly-selected slots in the array to 1 and then asserting that
a different randomly-selected set of slots are not equal to 2.  This
is the only part of the program which modifies the array, and the
array is initialised to zero, and so it is not possible for this
assertion to fail, but {\technique} must do some analysis to show
that.  \autoref{fig:eval:complex_alias_res} shows how the time taken
to perform the analysis varies with \texttt{nr\_loads} and
\texttt{nr\_stores}.
