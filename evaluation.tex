...
\begin{sanefig}
  ...
  \caption{Distributions of time taken by the per-crashing instruction
    steps of the analysis. \todo{No confidence indicator on failure or
      skip rates.  Also, they're not normalised against the experiment
      size.}}
...
\item The boxes above the PDFs give the number of potentially-crashing
  ...
  \begin{sanetab}
    ...
    \caption{Causes of failures during per-crashing instruction
      ...
      each step. \todo{Confidence intervals? Maybe express as
        ratios?}}
...
\begin{sanefig}
  ...
  \caption{Time taken by per-\gls{interferingthread} analysis steps,
    ...
    {\StateMachine}. \todo{Secondary mode running cross product is all
      the ones where the simplifiers could reduce it to a trivial
      {\StateMachine} which always survives; say that in text.}.}
...
\begin{sanetab}
  ...
  \caption{Causes of failures during per-\gls{interferingthread}
    ...
    each step. \todo{Maybe re-express as ratios so that I can add
...
\todo{Currently rerunning these experiments with a bigger timeout;
  update once I have more results.}
...
use for finding unknown bugs.  \todo{So why didn't I do that?}
