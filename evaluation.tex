...
\begin{itemize}
\item The performance breakdown stuff.
...
\item Section~\ref{sect:eval:real} applies the tool to some larger
  ...
  real bugs.
\item Section~\ref{sect:eval:alpha} shows the effects of the
  \gls{alpha} parameter, in terms of both the performance of the tool
  and the number of \glspl{verificationcondition} which must be
  checked at run-time.
...
\item Section~\ref{sect:eval:dynamic_analysis} investigates the
  ...
  running.
\item Finally, Section~\ref{sect:eval:time_details} then investigates
  the tool's performance on large programs in slightly more detail,
...
\section{Time spent in the various phases}
...
\item Similarly, some instructions do not start later phases because
  ...
  solid and dashed lines.  In this case, there was one timeout
  building the crashing {\StateMachine}, one simplifying it, and
  five deriving C-atomic.  All timeouts were after one minute.
\end{itemize}

Three important conclusions can be drawn from this chart:

\begin{itemize}
\item The most expensive phase of the analysis is processing the
  interfering \glspl{cfg}.  This will be analysed in more detail next.
\item The second most expensive phase is generating the C-atomic
  constraint.  This phase performs much of the work of determining
  which aliasing between thread-local accesses in the crashing
  {\StateMachine}, and so this reflects the fact that
  {\implementation} has only a very primitive model of the program's
  heap and global memory\footnote{The simplification phase also does
    some aliasing resolution, but usually defers the most complicated
    problems to the symbolic execution phase, and so does not see the
    same explosion in run-time when presented with a hard aliasing
    problem.}.
\item Most of the distributions show a quite bimodal pattern: a phase
  either completes very quickly, often too quickly to be easily seen
  on this chart, or it takes a very long time indeed.  Many of the
  algorithms used by {\technique} have worse than exponential worst
  case running time, and instructions which hit one of the worst cases
  effectively never complete while those which avoid them complete
  very quickly.
\end{itemize}
...
2157 interfering \glspl{cfg} in total.
...
Perhaps the most surprising part of this chart is that symbolically
executing the \gls{ic-atomic} {\StateMachine} is much more expensive
than symbolically executing the cross-product {\StateMachine}, even
though the cross-product {\StateMachine} is far more complex.  This is
in large part a selection effect: if the {\StateMachines} involve
complicated aliasing, deriving \gls{ic-atomic} is highly likely to
time out, and so {\implementation} will never reach the stage of
symbolically executing the cross-product {\StateMachine}.  To confirm
that hypothesis, I ran {\implementation} in a special mode which skips
deriving \gls{ic-atomic} and instead just uses C-atomic as the
\gls{inferredassumption}; the results are shown in
Figure~\ref{fig:eval:time_breakdown:interfering_no_ic_atomic}.  As
expected, the time taken processing the cross-product machine
increased noticeably so that the total reduction in processing time
was far less than the time taken deriving \gls{ic-atomic}.

\begin{figure}
  \input{eval/bubble_charts/bubble3.tex}
  \caption{Time taken by the different phases with \gls{ic-atomic}
    computation disabled.  See text for detailed description.}
  \label{fig:eval:time_breakdown:interfering_no_ic_atomic}
\end{figure}

\todo{More experiments to run here: figure out how much time we're
  spending in BDD processing, look at how long the different
  simplification passes take.}

The next thing to look at is how long it takes to convert these
\glspl{verificationcondition} to \glspl{bugenforcer}.  For this
experiment, I took the \todo{...} \glspl{verificationcondition}
generated by the previous experiment and converted each one to an
enforcer and timed how long each phase took; the results are shown in
Figure~\ref{fig:eval:time_breakdown:convert_to_enforcer}.  In this
case, the conversion process failed for one of the
\gls{verificationcondition}, due to reaching the one-minute timeout
whilst being sliced by the happens-before graph\footnote{I also tried
  running that one \gls{verificationcondition} with the timeout
  disabled and it did not complete within an hour.}.  This is not
completely surprising: as discussed in
\autoref{sect:enforce:slice_hb_graph}, the slicing algorithm has
extremely expensive worst-case behaviour but can almost always avoid
it in practice, and so is usually extremely cheap except for a small
number of cases in which it effectively runs forever.
...
\section{Effects of the $\alpha$ parameter}
...
\todo{These have no error bars, and they also have a quite large
  uncertainty.}
...
\todo{It might be interesting to include some numbers on how large the
  \gls{crashingthread} {\StateMachine} actually is; it's certainly a
  damn sight bigger than 40 instructions.}
...
dominate.  \todo{And there's a sudden increase in the seventy-fifth
  percentile on the very right without a matching increase in the
  mean; not sure quite what's causing that.}

\todo{I'd quite like to quantify how much of this is down to increases
  in the raw size of the {\StateMachines}, how much the more complex
  interleavings, and how much the aliasing explosion, but I don't have
  much idea how to actually do that.}
