...
\item That it could feasibly scale up to something of realistic size.
  ...
  \smh{Yes; this will be very nice.}
...
\todo{I might possibly be over-thinking this.  I may also be demonstrating
  that I don't really understand how stats works.}
...
discussed below have some performance metric\smh{So bigger is good?}
...
quite simple: $\frac{\textrm{baseline performance} - \textrm{modified
    performance}}{\textrm{baseline performance}}$\smh{You assume a
  clear denom?}.  Estimating the error bounds of this quantity is,
...
estimate has an infinitely large standard deviation\editorial{Possibly
  need a cite?}, which would be unhelpful.  It would still in
...
distributed\editorial{Don't actually have very much non-normal data,
  so I could just ignore it, but whatever.}.  Instead, I take the 80\%
confidence intervals for the two performance measures\footnote{Using a
  bootstrap method when necessary.}\editorial{Should make it clear
  that these are intervals for the \emph{means} rather than for the
  data.} and use these to calculate the minimum and maximum overhead
...
and $b$ is the simple ratio of means.  \todo{Von Luxberg and Franz
  2007 calls this a geometric bootstrap, but I don't think that's a
  terribly helpful way of describing it.}  \todo{Alternative would be
  to use a Hwang bootstrap to estimate confidence intervals on the
  ratio directly, but there have been a bunch of papers recently
  saying that that gives you intervals which are too tight when its
  preconditions don't fit, and I don't know enough stats to check
  whether they do here.}
...
\todo{\emph{Should} do all of that stuff.  Currently just report the
  ratio of means.}\smh{Probably ok for now.  Remember there's a lot of
  system + measurement noise.}

Crash time distributions are generally less well-behaved than
performance distributions.  Depending on the precise test\smh{BAd
  word! use ``run''?} being examined, I would expect the distribution
...
From a statistical perspective, this scheme is hard to justify, and
...
\smh{Most of this does not matter and is insufficiently precise
  anyway.  What benefit is there in having defined in a metric that is
  not decent??}
...
\todo{Perhaps I should move some of this discussion a bit earlier and
  treat it as an example?}\smh{Yes, could be a good plan!}
...
\end{verbatim}
    ...
    \caption{Write side}
  \end{subfloat}
  \caption{Disassembly of the program in Figure~\ref{fig:eval:simple_toctou}.}\smh{More clear separation}
...
\end{verbatim}
  ...
  \caption{Crash summary generated for the program in
    Figure~\ref{fig:eval:simple_toctou}.}\smh{Gory.  Can you relayout or ``redraw''?}
...
\item Control flow graph fragments for the two sides.  The read side
  ...
  fragments would\smh{hmm.  How is this not present in the actual
    {\StateMachines}?} include details of which nodes were created by
...
\item A memory access identifier table, showing how memory access
  ...
  nodes\smh{Hmm... really??  Why is this good?}.
...
for every summary-level one\smh{New terms!}.  Finally, the patch
...
time.\smh{So this somehow then turns into thing with message passing?}
...
\begin{figure}
  ...
  \caption{Enforcing the simple\_toctou bug.  Dotted lines indicate
    ...
    roughly 1.14 seconds.}\todo{Not sure this one adds all that much.}
\end{figure}
...
\item One second, which is caused by the call to \verb|sleep| in the
  test harness.
\item An additional 100 to 200ms\smh{Huh?}, which is caused by the RX
  message delay\smh{What is this?}.  The write thread starts its
...
\todo{I currently just call into libpthread to actually acquire and
  ...
  help to get the overhead numbers down.}\smh{Ok}
...
\todo{Give some breakdown of where the overhead comes from.  I
  ...
  and releasing the lock; it'd be nice to confirm that.}\smh{Yes}
...
\todo{Say something about how I set the analysis window.  It doesn't
  ...
  a sentence or two somewhere to say that.}\smh{Possibly need some
  much earlier, too!}
