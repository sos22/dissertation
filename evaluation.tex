...
deviation here was $5.0 \pm 5.1$\editorial{only ten runs; redo stats},
...
\todo{Numbers in the big table are actually from a moronic
  distribution resampler; need to do this properly.}
...
\todo{Like the simple\_toctou tests, the actual statistics were a bit
  different to that.}  \todo{Proper goodness-of-fit test on the
  exponential distribution?}

\todo{Thinking about it some more, I'm half convinced that the right
  thing to do here is just to take the reciprocal of the time taken.
  Although that's still not Gaussian, so I'd need to do some further
  massaging, and it definitely wouldn't be the right thing to do for
  simple\_toctou, so maybe best not to bother.}
...
The important properties of my DataCollider-like tool are:
...
parameters which I tried.  \todo{Should be more systematic about the
  parameter search there.}

\todo{Reading the DC paper again, this isn't actually all that
  similar: DC is only really a way of finding races, and not a
  particularly effective way of tweaking the schedule.  Not sure what
  I make of that.}
...
statistical sampling behaves well\editorial{Make that more clear.}.
...
\todo{I'd really like to do a comparison with CHESS, as well, but
  ...
  {\technique} is only implemented for 64-bit Linux ones, so that
...
a 2\% reduction.  That's not great, especially not given how expensive
it is, but it's not completely terrible either.
...
\caption{CDF of time taken to analyse instructions in an optimised
  ...
  analysis window of twenty instructions.}  \todo{Include some other
