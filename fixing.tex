...
\begin{sanefig}
  ...
  \begin{tabular}{ccc p{3.8cm} c}
    ...
    \hdashline
    \graphNT{\raisebox{-5mm}{$\left(\begin{tikzpicture}[baseline = (current bounding box.center)]
        \node (r) {\texttt{jcc}};
        \node (s) at (-.3,-.7) {X};
        \node (t) at ( .3,-.7) {Y};
        \draw[->,ifTrue] (r) -- (s);
        \draw[->,ifFalse] (r) -- (t);
      \end{tikzpicture}, a, l, \textsc{Emul}\right)$}} & $\Rightarrow$ & \begin{tikzpicture}[baseline = (current bounding box.center)]
      \node (r) at (.8,0) {\texttt{jcc}};
      \node (s) at (-1,-1) [style=graphNT] {
        \renewcommand{\arraystretch}{1}
        \hspace{-3.5mm}\raisebox{-5mm}{
          $\left(\!\!\!\begin{array}{l}%
            X, \mathit{adv}(X, a),\\
            l, \textsc{Unlock}\\
          \end{array}
      ...
      \draw[->,ifTrue] (r) -- (s);
      \draw[->,ifFalse] (r) -- (t);
    \end{tikzpicture} \renewcommand{\arraystretch}{1.5} & & \circled{$4_a$}\\
    \raisebox{5mm}{\graphNT{\raisebox{-5mm}{$\left(\begin{tikzpicture}[baseline = (current bounding box.center)]
          \node (r) {\texttt{jmp}};
          \node (s) [below = .3 of r] {X};
          \draw[->] (r) -- (s);
      \end{tikzpicture}, a, l, \textsc{Emul}\right)$}}} & $\Rightarrow$ & \graphNT{$(X, \mathit{adv}(X,a), l, \textsc{Unlock})$} & & \circled{$4_b$} \\
    \raisebox{5mm}{\graphNT{\raisebox{-5mm}{$\left(\begin{tikzpicture}[baseline = (current bounding box.center)]
          \node (r) {\texttt{call} X};
          \node (s) [below = .3 of r] {Y};
          \draw[->] (r) -- (s);
      \end{tikzpicture}, a, l, \textsc{Emul}\right)$}}} & $\Rightarrow$ &
    \begin{tikzpicture}[baseline = (current bounding box.center)]
      \node (r) {\texttt{push} X};
      \node (s) [below = .3 of r] {\graphNT{$(Y, \mathit{adv}(Y,a), l, \textsc{Unlock})$}};
      \draw[->] (r) -- (s);
    ...
    \begin{tikzpicture}[baseline = (current bounding box.center)]
      ...
      \node (s) [below = .3 of r] {\graphNT{$(Y, \mathit{adv}(Y,a), l, \textsc{Unlock})$}};
      \draw[->] (r) -- (s);
    \end{tikzpicture} & Other instructions $X$ & \circled{$4_d$} \\
    \multicolumn{4}{c}{\leaders\vrule height .5ex depth -.45ex\hfill \textit{Indirect control-flow described in text\!}
      {\leavevmode\leaders\vrule height .5ex depth -.45ex\hfill}
    } \\
    \labeledHLineA{\textit{Indirect control-flow described in text\!}}{\vrule height .5ex depth -.45ex} \\

    \hdashline
    \multirow{2}{*}{\graphNT{$(r, a, l, \textsc{Unlock})$}} & \multirow{2}{*}{$\Rightarrow$} & \begin{tikzpicture}[baseline = (current bounding box.center)]
      \node (r) {Release lock};
      \node (s) [style=graphNT, below = .3 of r] {$(r, a, \false, \textsc{CheckForEntry})$};
      \draw[->] (r) -- (s);
    \end{tikzpicture} & If $a = \varnothing$ and $l = \true$ & \multirow{2}{*}{\circled{5}}\\
    & & \graphNT{$(r, a, l, \textsc{CheckForEntry})$} & Otherwise \\
  \end{tabular}
\end{sanefig}
...
\todo{SMH \emph{hates} the interpreter-based description, but I find
  his reasoning deeply unconvincing.}
...
the program's inputs unspecified\editorial{Alternative way of thinking
  about this: We want to convert the interpreter into a JIT.  We copy
  the key insight from PIN that automorphic compilers should use an
  intermediate form which is similar to their output form and use an
  intermediate form which is just the original instruction plus an
  \textsc{InterpreterState}.  Given that, the JIT becomes sufficiently
  simple that we can just do the entire thing offline before we start,
  at which point we have a compiler.}.
\begin{itemize}
