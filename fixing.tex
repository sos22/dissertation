...
\noindent
The first step in producing such a fix is to infer some critical
sections of the dynamic \glspl{cfg} such that running those sections
atomically with respect to each other would be sufficient to eliminate
the bug.  These will not necessarily cover the entirety of the dynamic
...
which is necessary for the bug to reproduce.  Any instruction not
mentioned in such a test can be ordered arbitrarily without
...
$\happensBeforeEdge$-free side condition, and so the critical sections
need only include statements \texttt{2}, \texttt{4}, and \texttt{8},
plus those on paths between them (in this case, just statement
\texttt{3}).  \texttt{complex\_local\_condition()} is excluded, as
desired.
...
\texttt{8b}; in other words, whether to use two critical sections,
...
\noindent
Once {\technique} has identified the fragments of dynamic \gls{cfg}
...
machine code which emulates the interesting part of the original
program with the addition of appropriate lock acquire and release
operations; and the patch strategy, a set of modifications to the
original program which ensure that the shadow gains control of the
program when necessary.  The \gls{shadowfragment} is in many respects
analogous to the crash enforcement interpreter
(\autoref{sect:enforce:interpreting}): both gain and relinquish
control of program threads as they run; both are denotationally
...
dynamic \gls{cfg} indicates that the critical section must run from
...
\begin{wrapfigure}{o}{4.4cm}
  ...
  \caption{Lock protocol automaton.  The \texttt{jmp} state matches
...
\begin{sanefig}
  ...
  \begin{tabular}{ccc p{3.21cm} c}
    ...
    \begin{tikzpicture}[baseline = (current bounding box.center)]
      ...
      \draw[->] (r) -- (s);
    \end{tikzpicture} & X not a library function & \production{4_c} \\
    \raisebox{2.5mm}{\graphNT{\raisebox{-2.5mm}{$\begin{tikzpicture}[baseline = (current bounding box.center)]
    ...
    \begin{tikzpicture}[baseline = (current bounding box.center)]
      ...
      \draw[->] (r) -- (s);
    \end{tikzpicture} & x some other instruction type & \production{4_d} \\
  ...
  \caption{The patch generating algorithm as a graph grammar.  The
    ...
    production is used only if \protect\production{2_a} does not match.
...
The initial graph consists of a set of \textsc{Configuration}s given
in \autoref{fig:fix:initial_configuration}.  The
\textsc{Configuration} type is used to represent the program's
positions in the various regular automata:
\begin{itemize}
\item $\mathit{static}$ gives the position in the program's static
  \gls{cfg}.
\item $\mathit{dynamic}$ gives the set of possible positions in the
  dynamic \gls{cfg}.
\item $\mathit{holdLock}$ gives the position in the lock discipline
  automaton (either {\true} for the lock held state or {\false} for
  the lock unheld one).
...
\item $\mathit{adv}(\mathrm{x}, \alpha)$, of type
  ...
  $n$ and $n.\mathit{static}$ is the static \gls{cfg} node
  corresponding to $n$.
\end{itemize}
...
way in which it is generated (using boxed and double arrows).  The
initial non-terminal \graphNT{A, $\varnothing$, \false,
  \textbf{CheckForEntry}} is transformed by \production{1} to the
non-terminal \graphNT{A, $\varnothing$, \false, \textbf{Lock}}, which
is in turn transformed by productions \production{2_b} and
\production{3_b} into the non-terminal \graphNT{A, $\varnothing$,
  \false, \textbf{Emul}}.  Production \production{4_d} then transforms
this into the terminal instruction A followed by a further
non-terminal \graphNT{B, $\varnothing$, \false, \textbf{Unlock}}.
This new non-terminal is itself transformed by production
\production{5_b} and the process continues until all non-terminals
have been transformed.  The resulting graph forms the
\gls{shadowfragment}, as shown in \autoref{fig:fix:eg:shadow}.
...
\begin{sanefig}
  ...
  \caption{The effect of running the loop in
    ...
    instruction {\rm B} could be either $B_0$ or $B_1$ the dynamic
    \protect\gls{cfg} in \autoref{fig:fix:eg:dynamic} matches this
...
occurs twice in the dynamic \gls{cfg} as $B_0$ and $B_1$.  The
...
synchronisation\footnote{At the level of the graph grammar, this means
  that production \production{2_a} is extended to branch to
  \graphNT{X, $\varnothing$, \false, \textbf{Return}} when lock
  acquisition fails.}.  This risks re-introducing the original bug,
...
the fragment and leaving program-library ones as true function calls.
Note that this implies that the \gls{shadowfragment} will sometimes
acquire program-level locks whilst holding the {\technique} one, and
that this can sometimes lead to deadlocks.  As previously discussed,
these are detected as timeouts, and cause one or more threads to
proceed unprotected.
...
\item The branch might instead select an X such that
  ...
  generated\editorial{These two first cases are kind of redundant, but
    I think it's easier to understand if they're split out.}.
...
but could potentially cause a significant loss of performance if it
happens too often.  If necessary, these problems could be mitigated
using existing deadlock aversion techniques such as Gaddara\needCite{}
or Dimmunix\needCite{}, or even a simple adaptive timeout mechanism.
