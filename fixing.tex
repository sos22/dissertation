...
abandoning its attempt to impose the desired synchronisation\fnote{At
  the level of the graph grammar, this means that production
  \production{2_a} is extended to branch to \graphNT{X, $\varnothing$,
    $\mathit{unheld}$, \textbf{Return}} when lock acquisition fails.}.
This risks re-introducing the original bug, but ensures that
{\technique} will never introduce any new deadlocks.
