...
\todo{Should be more explicit that this helps mostly with stack-based
  accesses, and showing that a particular memory access does or does
  not go to the stack.}
...
\item
  ...
  starts.\smh{Find ref / add a flow chart?}\editorial{There's a flow
  chart in the introduction now; find proper refs.}

  \todo{I'm using the model in a couple of other places, so it perhaps
    doesn't want to be part of the alias analysis section.  On the
    other hand, the alias analysis is the main consumer, and the most
    important part of the model is the points-to table.}
...
\item
  ...
  \todo{Ref the section on build {\StateMachines}, since there's a
    forward ref to this from there.}
\item
  The main alias analysis\editorial{Name?} can then be run on the
