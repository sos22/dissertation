...
\item[W isolation] In all of the executions considered, $W$ does not
  load from any locations which are stored to by $R$.\smh{Define
    readset and writeset? (+relabel R+W as something else?)}
\end{itemize}
...
completely ignore them here.\smh{!sarcy}
...
\end{verbatim}
  ...
  \caption{A fragment of machine code, and the {\StateMachine}
    ...
    lines leaving an \state{If} state indicate the false branch; solid
    ones indicate the true branch.}\smh{Very ugly
    figure!}\smh{Captions in italics?}
\end{figure}
...
\begin{sidewaysfigure}
  \begin{tikzpicture}
    ...
    \node (lN) [stateSideEffect, below = of lF] {\state{Assert} $\&global\_ptr = \&global\_ptr$\smh{Huh?} };
...
\item
  Paths in which either the store or load machine end\smh{ok terms}
...
\item
  ...
  hence restrict the cross-product {\StateMachine}\smh{This is a nice
    concept, wonder if it needs a better name?}.  They are used both
...
\subsection{Building read-side \StateMachines}\smh{read-side -- hmm}

The aim of this phase of the algorithm is to find every fragment of
the program which might act as the first ($R$)\smh{You had (P,R,W).}
...
idea here is to start with the\smh{fussy! what instr is this?  is
  there just 1???}\smh{``an instruction''?} instruction at which the
...
\caption{Building a read-side static control flow graph within a
  single function. \todo{This is just depth-limited BFS; maybe drop
    the explicit algorithm?}}\smh{No, but you can make owls to clarify
  this in text (as you do).}
...
\item
  The AMD64\smh{Are you being pedantic here?  Does ``x64'' work as a
    replacement?} instruction set includes variable-length
...
trace\smh{Not 100\% clear what you mean by this -- is this an actual
  trace, or another word for the slice or CFG.  Imprecision makes this
  stuff harder to read!} will be contained in a single function, and
...
has been inlined\editorial{Not terribly clear.}.  The only slight
...
The result of the previous phases\smh{Would be nice to refer back to a
  figure (signpost) here!} is a control-flow graph containing all of
...
isolation and convert it into an appropriate\smh{hmm} fragment of
\StateMachine, and then stitch all of the fragments together again in
the obvious\smh{hmm} way.  The unrolling process introduces three
major complications:
...
\caption{{\STateMachine} generated from the CFG shown in
  Figure~\ref{fig:cyclic_cfg}.}\editorial{Check this very carefully.}
...
the same memory location, and this is impossible\smh{intractable} in
...
the style of \editorial{cite Van Emmerik 2007}; {\technique}'s alias
analysis pass, described in Section~\ref{sect:alias_analysis} can be
seen as doing something very similar to this.\smh{This is a bit
  rambling -- what do you want to say?}
...
The definition of bugs given in
section~\ref{sect:finding_bugs:finding_candidate_bugs:formal_definition}
is existentially quantified over all write thread sequences of length
$N_w$ instructions\smh{Is it sensible to define $N_r$ and $N_w$
  earlier? + to explain that they can be different?  I had assumed
  analysis window was one param...}.  As with the read-side
...
is only interesting if it can influence the behaviour of the\smh{`a'?
  Or do you generate write side slices separately for each read-side
  slice?} read-side {\StateMachine} in some way, which means that the
...
location from another thread\smh{is this complete?}, and so find all
...
one of these interfering accesses\smh{This means an access in W which
  may be read by some R insn? Or the last R insn?}\smh{Same loc or
  different?}, and this is usually a far smaller set.
...
instructions according to some heuristics\editorial{Explain those
  heuristics somewhere.}.  \todo{Not sure how clear that para
  is.}\smh{Not very clear at all.}\smh{Some more formalism might help,
  including definition of interesting; of prefix interesting is just
  fine, but the prefix does not value due to W isolation (+ ring it
  contains no interesting store).}
...
\item
  ...
  some loads to be removed.  \todo{Do I need an example here?
    ...
    sound a bit mysterious.}\smh{Maybe just explain that there
    is an undecorated version of the slice which you use.}\smh{Definitions +
    abstraction = good for documents, too!}
\item
...
\item
  ...
  might access the same location.\editorial{Arguably a little
    deceptive: a single instruction can be sometimes private and
    sometimes non-private, and the analysis correctly handles that.
    Maybe that's too complicated to explain?}
\end{itemize}
...
relevant static store instructions\smh{Who are they?}, and the
...
to five\smh{Not clear what this is.  You mean: shortest cyclic path
  starting at A}.  Now duplicate B because of the A1 to B
...
\item
  A write-side {\StateMachine} which terminates\smh{What does this
    mean precisely?  You have write side beginning with an interfering
    store, right??}\editorial{Oops, forgot to describe the bit where
    we expand the write side backwards from the first store to get
    some potentially-useful context.} without issuing any
...
\end{verbatim}
   ...
   optimisations.  \todo{I should really come up with a better example
     here.  Or just not bother mentioning this part.}\smh{Ack.  It's
     too long for what it brings to the argument.}
\end{itemize}
...
\begin{algorithmic}[1]
\State $cdg[root] = const(true)$\smh{Presumably, you meant to italicise cdg itself to something?}
\While {Some state is unlabelled in $cdg$}
  ...
  \State {$cdg[n] \gets \bigvee \{cdg[p, n] \mid p \in \textrm{$n$'s predecessors}\}$}\Comment{Take union of all reaching paths}\smh{Maybe use a different font for comments?}
\EndWhile
\end{algorithmic}
...
\smh{The difference between a state and a node is???}
...
because the {\StateMachine} is acyclic\editorial{Is that obvious?  Or
  do I need to go and prove it?}\smh{Yes, it's obvious (i.e. no need
  for proof); however the main text should remain (i.e. worth
  mentioning).}.  This assumption is also the reason that
...
\smh{Very unclear! example?  Isn't this effectively just computing the
  path condition?}
...
\todo{Do I need to give the \textsc{flatten} algorithm as well?  It's
  even more tedious and annoying than these ones.}
...
\item
  ...
  starts.\smh{Find ref / add a flow chart?}
\item
...
divided into eight byte chunks\smh{Why is 8 bytes ok?  c/f Andy's
  cache take chastity papers.}, each of which has a label consisting
...
itself to the relevant set\smh{May be worth mentioning something about
  simplified instruction at this stage, or do you do this with full
  x86 insn?  What about RMW insn?}\smh{How does this work in practice?
  Binary rewriting a la PIN?  valgrind?  custom analysis?  How
  slow/fast?  What about repeared accesses?  Assume ok as they'll be
  hot in registers??}.  These labels in effect identify the field for
...
\item
  ...
  cost of the analysis\smh{Why?}, and most such accesses are easily
  resolved using other analysis mechanisms, and so doing so would not
  actually be helpful. \todo{Need to eval how much that buys us.  In
    general, need to eval completeness and overheads of the dynamic
    model\smh{Ack}.  Also, this isn't just a performance tweak, but
    avoids a lot of nastiness to do with tail calls, since they kill
    off the frame but aren't obvious in the instruction stream.}
\item
  ...
  the region is marked as being thread-shared\smh{So even if we malloc
    an array of foo $\ast$s then malloc a bunch of foos and point to
    them from the array?}\footnote{The dynamic analysis checks every
...
each function in the program, before the main analysis\smh{flow chart
  showing ``main analysis''? (with a better name!)} starts, to
...
information by extending the analysis window\smh{Is thi sthe same
  window as was used to build the r-side and w-side slices? Been a
  while since we talked about this.}, but this is often expensive, and
...
\item
  ...
  usually a problem.  \todo{But it can be sometimes.  It'd be nice to
    know how often, and how bad it is when it does happen.}\smh{How
    hard to detect?  Is there an explicit allocal or yuan?}
...
\item
  ...
  highly non-portable for little practical benefit.\editorial{Rephrase
    a bit.}
...
\item
  ...
  \item They must not attempt to calculate pointers into the calling
    ...
    does not affect their behaviour\editorial{Could be
      clearer.}.\smh{Doesn't add much (as written, anyway).}
  \end{itemize}
...
\item
  ...
  concern\editorial{Might be worth asking on some compiler mailing
    lists to make sure I haven't missed anything.}\smh{Yes, sounds good}.
\end{itemize}
...
rules.\smh{explain better i.e. PA[<reg>], PA[STACK], PA[NONSTACK] PA
  $\rightarrow$ p[STACK,NONSTACK]}
...
\item
  ...
  \item $\varnothing \rightarrow \varnothing$ \todo{Do I need to
    explain that a bit more?}\smh{Ugly and not super
    clear!}\smh{Define ``stack'' and ``nonstack'' first?}
  \end{itemize}
...
\item
  ...
  it is set to $\varnothing$.\smh{Fix layout.}
\item
...
\todo{Frame ID assignment is done over the entire {\StateMachine},
  ...
  entry point.  Not sure I make that terribly clear at the
  moment.}\smh{I think it's reasonably clear.  Certainly a lot clearer
...
\smh{Float?  And label as an example?  Could combine both examples as
  (a) + (b) on the lhs and rhs of single float...}
...
executions.  The task of this pass\smh{undef} is to replace these placeholders
...
using special \state{StackLayout} side-effects\smh{What do these look
  like?} at every entry point and by attaching the relevant frame ID
to each \state{StartFunction} and \state{EndFunction} side
effect\editorial{Be more careful about what entry point means.}.
...
form.\smh{Example for the program fragments shown on previous page?
  e.g. show how the ids would be computed? and what they would be?}
...
\item
  Which registers might contain pointers to the frame.\smh{??}
\end{itemize}

Suppose the call stack consists of functions $f$, $g$, and $h$, where
$h$ is the function executing when the {\StateMachine} starts.  The
static analysis then makes determining this information\smh{What
  information?  the load 3 datums?} for $h$'s frame trivial, as this
is precisely the contents of $h$'s instruction label\smh{h is a
  function, not an instruction}\smh{Do you mean before(I) or
  after(I)?}\editorial{\emph{Almost} precisely, anyway}, but $f$ and
...
\todo{Good God I'm making a meal of explaining this.}\smh{Yes.}

\smh{Very confused now!! Have I got the call stack backwards??  Using
...
memory-accessing side effects to a set of side-effects\smh{hmm
  ``instructions'' might be clearer here, even if not strictly
  accurate.} with which they might alias.  They are built inductively,
starting with conservative\editorial{Why?  Should be able to use
  aggressive tables safely, and the results would probably be better.}
...
\todo{The actual approach is a bit less stupid than this and avoids a
  ...
  with conflicting initial points-to configurations?}\smh{As before,
...
analysis.\smh{Ah -- text is becoming a bit scrappier, it seems.}
...
never happens. \todo{Need to argue this a bit more
  carefully.}\smh{Hmm.}
...
\item
  ...
  frames.\smh{So what?  They have no effect anyway, right?  Generally:
    use of + meaning of these ``side effects'' is unclear to me +
...
\todo{I'm kind of tempted to kill this section off.  It's kind of
  neat, but it's not \emph{that} useful, and its novelty-to-fiddliness
  ratio isn't brilliant.}
...
two side effects\smh{unc! !!!} which we wish to unify:
...
\todo{Should probably have a more realistic example, really.}
...
\smh{Quite a lot of text for something that may or may not be
  important...  Hmm.  Does this unification help with real progs?}
...
the map from instructions to exit labels\smh{Long sentence}.  A few
