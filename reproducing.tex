...
\begin{sanefig}
  {\hfill}
  \tikzstyle{CfgInstr}+=[draw, text width=7.3cm, fill=white]
  \newcommand{\localcenter}[1]{{\hfill}#1{\hfill}~~}
  \newcommand{\mklabel}[2]{\texttt{L#1:\localcenter{#2}}}
  \hspace{-5mm}
  \begin{tikzpicture}[node distance = 1.2 and 0.1]
    \node[CfgInstr] (l1) {\mklabel{1}{ADD global\_ptr + idx1 $\rightarrow$ reg1}};
    \node[CfgInstr, below=of l1] (l2) {\mklabel{2}{LOAD {$\ast$}reg1 $\rightarrow$ reg2~}};
    \node[CfgInstr, below=of l2] (l3) {\mklabel{3}{CMP 0, reg2~}};
    \node[CfgInstr, below=of l3] (l4) {\mklabel{4}{JMP\_IF\_EQ~~}};
    \node[CfgInstr, below=of l4] (l5) {\mklabel{5}{LOAD {$\ast$}reg1 $\rightarrow$ reg3}};;
    \node[CfgInstr, below=of l5] (l6) {\mklabel{6}{STORE 7 $\rightarrow$ *reg3}};
    \node[CfgInstr, below=of l6] (l7) {};
    \node[CfgInstr] at (7,-.92) (l8) {\mklabel{8}{ADD global\_ptr + idx2 $\rightarrow$ reg4}};;
    \node[CfgInstr, below=30.5mm of l8] (l9) {\mklabel{9}{STORE 0 $\rightarrow$ *reg4}};
    \draw[->] (l1) -- (l2);
    \draw[->] (l2) -- (l3);
    \draw[->] (l3) -- (l4);
    \draw[->,ifFalse] (l4) -- (l5);
    \draw[->,ifTrue] (l4.west) .. controls +(-.3,0) and +(0,.3) .. ++(-.5,-.5) -- ++(0,-4.4955) .. controls +(0,-.3) and +(-.3,0) .. ++(.49,-.5) -- (l7.west);
    \draw[->] (l5) -- (l6);
    \draw[->] (l6) -- (l7);
    \draw[->] (l8) -- (l9);
    \draw[->,happensBeforeEdge] (l2.east) -- ++(.4,0) .. controls +(.3,0) and +(0,.3) .. ++(.5,-.5) -- (node cs:name=l9,angle=172);
    \draw[<-,happensBeforeEdge] (l5.east) -- ++(.4,0) .. controls +(.3,0) and +(0,-.3) .. ++(.5,.4) -- (node cs:name=l9,angle=-172);
  \end{tikzpicture}
  {\hfill}
  \caption{CFG with happens-before edges for the example bug in
    \autoref{fig:enforce:example_threads}.  Dashed lines are
    happens-before edges, solid lines give normal control flow, and
    dotted lines give control flow when a conditional test fails.}
  \label{fig:using:example_hb_graph}
\end{sanefig}

Consider, for example, the program threads shown in
...
\item
  ...
  forming the final \gls{bugenforcer}\editorial{ugg}.
\end{itemize}

\section{Extracting the happens-before graph}
\label{sect:enforce:slice_hb_graph}

\todo{Not sure how clear this bit is.}
...
\Technique{} implements this factorisation as a transformation on the
...
\begin{sanefig}
  ...
  \caption{Semantically equivalent BDDs which both represent the
    \glsentrytext{verificationcondition} for the bug illustrated in
    \autoref{fig:reproducing:placement_eg_bug}.  The representation on
    the right is easily factorisable into happens-before graphs and
    side conditions; the one on the left is not.}
  \label{fig:reproducing:slice_eg}
\end{sanefig}

This is a potentially expensive algorithm; re-ordering the BDD
...
\begin{sanefig}
  \begin{tikzpicture}
    ...
    \node[left = 4cm of D] (D1) {$D_1$};
    \path (D1.north) ++(.5,0) node [above] {\shortstack[l]{$\controlEdge{1}{C}{D_0}$,\\$\textsc{rax}_1$, $\textsc{rcx}_1$\\$\textsc{rbx}_2$, $\textsc{rcx}_2$}\!\!\!};
    \node at (t2 |- A) (X) {X};
...
\begin{sanefig}
  \centerline{
    \begin{tabular}{ccccc}
      ...
      Reordered BDD & \multicolumn{3}{c}{Condition evaluable at $N$} & Residual condition\\
    \end{tabular}
  }
  \caption{Factorisation of the BDD in
...
\begin{sanefig}
  \begin{tikzpicture}
    ...
    \node[below = of C] (D) {$D_0$};
    \node[left = 4cm of D] (D1) {$D_1$};
    \node[above = 0 of D1] {\shortstack{$\controlEdge{1}{C}{D_0} \vee$\\$\textsc{rbx}_2 = 72$}};
    \node at (t2 |- A) (X) {$X$};
    \node at (t2 |- B) (Y) {$Y$};
    \node at (t2 |- D) (Z) {$Z$};
    \draw[->] (A) -- (B);
    ...
    \draw[->,happensBeforeEdge] (Z) to node [above] {N} node [below] {\shortstack{$\controlEdge{1}{C}{D_0} \vee$\\$\textsc{rbx}_2 = 72$}} (D);
  \end{tikzpicture}
...
\begin{sanefig}
  ...
  \begin{tikzpicture}
    ...
    \node (n14) [below = 1.5 of n13, CfgInstr] {Receive message 1};
