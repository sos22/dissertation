...
\todo{Could also do with a cite of \cite{Khoo2010} i.e. the hybrid
  type checking/symbolic execution thing.}  \todo{Need to cite
  \cite{Cavada2007} and \cite{Graf1997} i.e. something about predicate
  abstraction.}

\todo{Steve says that the related work should be a hatchet job on
  other people's attempts at solving the same problem, explaining
  what's wrong with them.  I disagree; I think it should be a balanced
  attempt to compare existing systems with the one I've described, and
  if there's something which someone else does better then that
  definitely goes in.}

\todo{Linkage text goes here.}
...
and relevant are summarised in \autoref{fig:rw:find_char}.  They are
...
system\cite{Engler2001}\editorial{ugg} or MUVI\cite{Lu2007}.  The idea
...
area\editorial{ugg}.  In this approach, the tool compares traces taken
...
%% as a kind of predicate abstraction problem\cite{Graf1997}.  Cavada et
...
%% completely unevaluatable, which can sometimes force them to
