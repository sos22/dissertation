...
\todo{Need to be more explicit that it's a DAG of states.}
...
canonicalising general expressions is impossible\editorial{Less
  convinced of that than I used to be.}, and so while ordinary BDDs
are canonical representations of boolean functions those used by
{\technique} are not.  This lack of canonicalisation can sometimes
lead to poor performance if a single function is represented in many
different forms.  Fortunately, a few very simple canonicalisation
rules, such as sorting the arguments to commutative operators and
respecting transitivity of equality, usually suffice to reduce
redundancy to an acceptable level and avoid the worst performance
problems.  These are discussed further in
...
find its \gls{cfg}.  The algorithm for doing so is shown in
...
\todo{Could maybe give the implementations of pthread\_mutex\_lock and
  pthread\_mutex\_unlock here, as well.  They're not very interesting,
  but it wouldn't hurt to have them somewhere.}

\subsection{The W isolation assumption}
...
\item Unlike most symbolic execution engines, the one used by
  ...
  redundancy\editorial{Should maybe say something about the flat model
    of memory meaning that I can't really do much CEGAR stuff?}.
\end{itemize}
...
\subsubsection{Indirect control-flow transfer instructions}

Indirect control-flow transfer instructions are those which set the
instruction pointer to a calculated value; for the AMD64 instruction
set, this includes indirect jumps, indirect calls, and return
instructions.  They are difficult to handle in the compiler as the
target is not available at compile time and the compiler must ensure
that it generates code for every state which might be reached, along
with appropriate branches to activate that code at appropriate times.

I consider only indirect jumps here; generalising to indirect calls
and return instructions is then easy given the considerations of the
previous section\editorial{Rephrase.}.  There are three cases which
