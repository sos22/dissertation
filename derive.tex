\chapter{Deriving the verification conditions}
\label{sect:derive}
...
SAL\cite{Howard2006}\editorial{That's citing completely the wrong
  thing!} work with source code and hence (mostly\editorial{But not
  always; dig up some references.}) assume that such information is
...
reduction in the total time to analyse a program.  \todo{Could maybe
  give an explicit program slicing reference here?}

\subsection{Stack canonicalisation}

\todo{Need to eval how effective this is.  Could just drop it.}
...
\todo{Possibly interesting: this means that once you've converted to
  ...
  at the \emph{end} of the {\StateMachine}, whereas for every other
  register it refers to the value at the \emph{beginning}.}
...
\item Aliasing queries are resolved lazily.  This means that if the
  ...
  \todo{I want to say that's faster, and I genuinely believe that it
    ...
    Also, might need a better explanation of that. Need to check how
    novel this is.  I suspect it's been done before.}
