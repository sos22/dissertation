...
It is important to emphasise at this stage\editorial{ugg} that
...
different if the {\StateMachine} has issued any \state{Store}
...
\begin{sanefig}
  \newcommand{\nodewidth}{7cm}
  \newcommand{\nodeheight}{1cm}
  \begin{tikzpicture}[minimum width = 6.9cm, minimum height = 1cm]
    \node (lA) [stateIf] { \!\!\!\stIf{\happensBefore{\mai{L1}{thread1}}{\mai{L7}{thread2}}}\!\!\! };
    \node (lB) [stateSideEffect, below = of lA] { L1: \stLoad{1}{\mathrm{global\_ptr}} };
    \node (lC) [stateSideEffect, right = of lB] {L7: \stStore{0}{\mathrm{global\_ptr}} };
    \node (lD) [stateIf, below = of lB] { L2: \stIf{\smTmp{1} = 0} };
    \node (lE) [stateTerminal, below = of lC] { \stUnreached };
    \node (lF) [stateIf, below = of lD] {\!\!\!\stIf{\happensBefore{\mai{L4}{thread1}}{\mai{L7}{thread2}}}\!\!\! };
    \node (lG) [stateTerminal] at (lE |- lF) {\stSurvive};
    \node (lI) [stateSideEffect, below = of lF] {L7: \stStore{0}{\mathrm{global\_ptr}} };
    \node (lH) [stateTerminal] at (lE |- lI) {\stUnreached};
    \node (lJ) [stateSideEffect, below = of lI] {L4: \stLoad{2}{\mathrm{global\_ptr}} };
    \node (lK) [stateIf, below = of lJ] { L5: \stIf{\smBadPtr{\smTmp{2}}} };
    \node (lL) [stateTerminal, below = of lK] { \stCrash };
    \node (lM) [stateTerminal] at (lE |- lL) { \stSurvive };
    \draw[->,ifTrue] (lA) -- (lB);
...
\begin{sanefig}
  \begin{center}
  \begin{tikzpicture}[minimum width = 7cm, minimum height = 1cm]
    ...
    \node (lC) [stateTerminal] at (-4,-4){\stSurvive};
    \node (lD) [stateTerminal] at (4,-4) {\stCrash};
    \draw [->] (lA) -- (lB);
    \draw [->,ifTrue] (lB) -- (lC);
    \draw [->,ifFalse] (lB) -- (lD);
  \end{tikzpicture}
...
\begin{tikzpicture}[minimum height=1cm, minimum width=6.5cm]
  ...
  \node[stateIf,below = of l2, inner sep = -5mm] (l3) {\!\!\!$A_0'$: \stIf{\controlEdge{threadId}{A_0}{B_0}}\!\!\!};
...
\begin{sanefig}
  ...
  \begin{tikzpicture}[align=center, node distance = 1 and 0.9]
    ...
    \labelBox{2.8}{-14.7}{4.8}{1.8}{1}{(\{$H$, {\true}\}, \{$G$, {\false}\})}
...
\section{Discussion}
\todo{Some kind of closing text goes here.}
