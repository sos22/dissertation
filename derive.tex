...
understood as a graph generating node-replacement grammar using
{\StateMachine} states as terminals and \textsc{Configuration}s, shown
in \autoref{fig:cross_product:configuration}, as non-terminals.  The
grammar starts with a single non-terminal, shown in
\autoref{fig:cross_product:initial}, and then uses the rules shown in
\autoref{fig:cross_product:algorithm} to expand that initial
non-terminal into a new graph.  The rules should be read as
pattern-matching systems: whenever a non-terminal matches one of the
patterns to the left of the $\Rightarrow$, it is replaced with the
graph on the right, substituting in variables as
appropriate\editorial{bit of an unbound reference.}.  This might
generate additional non-terminals, in which case the process repeats
until no more non-terminals remain.  If the same non-terminal is
generated several times then they are merged, so that the non-terminal
has one incoming edge for each position in which it is generated.  The
resulting graph of {\StateMachine} states then forms the cross-product
{\StateMachine}\editorial{That's a bit dense.}.

The rules are individually quite simple:
\begin{itemize}
\item The first two rules are trivial and simply show how to process
  ...
  represent what the crashing {\StateMachine} does next.  Operations
  local to the interfering {\StateMachine} are handled symmetrically.
  The only slight subtlety lies in determining whether an operation is
  local to a particular {\StateMachine}; in other words, whether it
  can influence or be influenced by the other {\StateMachine}.  Most
  side-effects only depend upon and only modify the {\StateMachine}
  temporaries, and so 

\item The second rule shows how to handle \state{If} states in the
  crashing {\StateMachine}.  The non-terminal on the left of the
  $\Rightarrow$ will match any \textsc{Configuration} in which the
  crashing {\StateMachine} is at an \state{If} state and will generate
  the graph fragment shown on the right.  This consists of a new
  (terminal) \state{If} state whose successors are new (non-terminal)
  \textsc{Configuration}s corresponding to the test in the crashing
         {\StateMachine} evaluating to either {\true}, on the left, or
         {\false}, on the right.  The rule for handling \state{If}
         states in the interfering {\StateMachine}, not shown in the
         figure, is symmetrical.

\item The first three rules implement atomic blocks in the crashing
  {\StateMachine}.  Rule \circled{1} says that the crashing
  {\StateMachine} can start a new atomic block if it is currently at a
  {\stStartAtomic} state (because the $\mathit{crashingState}$
  component of the \textsc{Configuration} on the left begins
  {\stStartAtomic}), neither {\StateMachine} is currently in an atomic
  block (because the $\mathit{atomic}$ component is $\varnothing$),
  and the atomic block cannot race with the current interfering
  {\StateMachine} state (because the condition on the right requires
  $C$ to be local; the precise definition of local is given later).
  Starting an atomic block means discarding the {\stStartAtomic}
  side-effect and setting $\mathit{atomic}$ to $\mathit{crashing}$.
\end{itemize}
...
\vspace{-1cm}
  \begin{displaymath}
    \begin{array}{cccp{5cm}c}
      ...
      \end{tikzpicture}}, i_a\right\},\left\{B, i_b\right\},z\right) & \!\!\!\Rightarrow\!\!\! & \hspace{-5mm}\raisebox{-8mm}{\begin{tikzpicture}[node distance=0.5cm,font=\small]
          ...
          \draw[->,ifTrue] (r) -- (A);
          \draw[->,ifFalse] (r) -- (B);
        \end{tikzpicture}}\hspace{-5mm} & \hspace{-3mm}\raisebox{2mm}{\parbox{5.2cm}{Likewise for interfering thread.}} & \circled{1}\\
      ...
      \left(\left\{\raisebox{-6mm}{\begin{tikzpicture}[font=\small]
        ...
        \end{tikzpicture}}, i_a\right\},\left\{B, i_b\right\},z\right) & \!\!\!\Rightarrow\!\!\! & \raisebox{-8mm}{\begin{tikzpicture}[font=\small]
          ...
          \draw[->] (r) -- (A);
        \end{tikzpicture}} &\hspace{-3mm}\parbox{5.2cm}{If $a$ is a local side-effect.  Likewise for interfering thread.} & \circled{2}\\
      ...
      \left(\left\{\raisebox{-8mm}{\begin{tikzpicture}[font=\small]
        ...
        \end{tikzpicture}}, i_b\right\}, \varnothing\!\right) & \!\!\!\Rightarrow\!\!\! & \hspace{-5mm}\raisebox{-22mm}{\begin{tikzpicture}[font=\small]
          ...
          \draw[->] (A) -- (B);
          \draw[->] (C) -- (D);
        \end{tikzpicture}}\hspace{-5mm} & \hspace{-3mm}\parbox{5.2cm}{When $a$ and $b$ are non-local.} & \circled{3}\\
      ...
      \end{tikzpicture}} & \hspace{-3mm}\parbox{5.2cm}{If $i_a \wedge i_b = \false$.  Likewise for interfering thread.} & \circled{4}\\
      
      \left(\left\{\raisebox{-3mm}{\begin{tikzpicture}[font=\small]
          \node [stateTerminal] {{\stCrash}};
      \end{tikzpicture}}, \true\right\}, \{A, \true\}, z\right) & \!\!\!\Rightarrow\!\!\! & \raisebox{-3mm}{\begin{tikzpicture}[font=\small]
          \node [stateTerminal] {{\stCrash}};
      \end{tikzpicture}} &  & \circled{5} \\
      
      \left(\left\{\raisebox{-3mm}{\begin{tikzpicture}[font=\small]
          \node [stateTerminal] {{\stSurvive}};
      \end{tikzpicture}}, \true\right\}, \{A, \true\}, z\right) & \!\!\!\Rightarrow\!\!\! & \raisebox{-3mm}{\begin{tikzpicture}[font=\small]
          \node [stateTerminal] {{\stSurvive}};
      \end{tikzpicture}} &  & \circled{6} \\
      
      \left(\left\{\raisebox{-8mm}{\begin{tikzpicture}[font=\small]
          \node at (0,0) (r) [stateSideEffect] {{\stStartAtomic}};
          \node at (0,-10mm) (A) {$A$};
          \draw[->] (r) -- (A);
        \end{tikzpicture}}, i_a\right\},\left\{B, i_b\right\}, \varnothing\!\right) & \!\!\!\Rightarrow\!\!\! & (\{A, i_a\}, \{B, i_b\}, \mathit{crashing}) & \hspace{-3mm}\parbox{5.2cm}{Likewise for interfering thread.  Additional condition in text.} & \circled{7}\\

      \left(\left\{\raisebox{-8mm}{\begin{tikzpicture}[font=\small]
          \node at (0,0) (r) [stateSideEffect] {{\stEndAtomic}};
          \node at (0,-10mm) (A) {$A$};
          \draw[->] (r) -- (A);
        \end{tikzpicture}}, i_a\right\},\left\{B, i_b\right\}, \mathit{crashing} \right) & \!\!\!\Rightarrow\!\!\! & (\{A, i_a\}, \{B, i_b\}, \varnothing ) & \hspace{-3mm}\parbox{5.2cm}{Likewise for interfering thread.} & \circled{8}\\
      ...
      \left(\left\{\raisebox{-8mm}{\begin{tikzpicture}[font=\small]
        ...
        \end{tikzpicture}}, i_a\right\},\left\{B, i_b\right\},\mathit{crashing}\right) & \!\!\!\Rightarrow\!\!\! & \raisebox{-8mm}{\begin{tikzpicture}[font=\small]
          ...
          \draw[->] (r) -- (A);
        \end{tikzpicture}} & \hspace{-3mm}\parbox{5.2cm}{Likewise for interfering thread.} & \circled{9}\\
  ...
  \caption{The cross product algorithm as a node replacement graph
    ...
    $m$ matches a boolean BDD.  \state{T} matches any terminal state.}
...
\item
  ...
  \state{Store} ones.  This grows rather quickly in the number of
