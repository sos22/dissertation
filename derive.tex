...
\caption{Fully unrolled version of the CFG in
  ...
  less. \smh{Not refed from text here (although it is used much later)}}
...
\smh{Should be clear that this is less relevant to 2.1.}
...
\begin{sanefig}
  ...
  \caption{Small-step semantics for single-thread {\StateMachines};
    ...
    environment $c$ and variables $t$.  \smh{I was surprised that
      false asserts eval to UNREACHED rather than CRASH} \smh{What's
      the difference between the speicher and the memory?}}
...
behaviour.\kern-.2pt\fnote{Even with a more complete {\StateMachine}
  library implementation, {\technique} would still be a poor choice
  for investigating environmentally-dependent bugs:
...
structure, but instead simply {\stAssert}s that it is not currently
...
\item \emph{Copy propagation}, based on the algorithm from the
  ...
  {\technique} can make use of {\stAssert} side-effects during this
...
provide useful hints to the analysis.  \smh{Would it ever be useful to
  increase number of paths as well?}
...
\noindent
\smh{I didn't get this example at all.} As an example, consider this cyclic \gls{cfg}:\\
\centerline{
\inlinebox{
\begin{tikzpicture}
...
cyclic paths in the graph, and so \textsc{findCycleEdgeCrashing} will
report the two length four paths $A_0 \shortrightarrow B_0
...
is eliminated by once more duplicating $B_0$: \\
\noindent
\centerline{
\inlinebox{
\begin{tikzpicture}
...
\item The basic grammar fails to take account of partial order
  ...
  \state{Copy} or \stAssert.  The complete grammar ameliorates this
...
means that it is never necessary to do so.  \smh{what does safe mean
  here?}
...
\smh{not sure that categorising false positives as the most important
