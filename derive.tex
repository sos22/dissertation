...
algorithms used by {\technique} also have exponential worst-case
...
{\technique} takes, on average, a few seconds to process each
...
identified depends on the type of bug which is to be investigated.  My
prototype implementation considers three types of bug:
\begin{itemize}
...
\item Double free errors.  These are caused by the program calling
  ...
  information allows {\technique} to quickly find all calls to
...
\item Bad pointer dereferences.  Any memory-accessing instruction
  could potentially dereference a bad pointer, and so {\technique}
  simply enumerates all memory accessing instructions discovered by
...
An analysis which fails 0.1\% of the time would still be quite useful,
and so this is reasonable for analysing specific bugs.  On the other
hand, if the analysis is run 10,000 times then the probability of at
least one of the steps taking forty years is very close to one and the
analysis is effectively useless.
...
encounters one of its bad cases.  {\Technique} uses two independent
timeouts, one for per-\gls{crashingthread} work and the other for
per-\gls{interferingthread} work, which, in my implementation, are
both set by default to five minutes.  I discuss their effects in more
detail in the evaluation.
...
and~\ref{fig:state_machine_exprs}.\fnote{My implementation includes
  several features, such as sub-word memory accesses and bounded-range
  integers, not given in these figures, but these are not essential to
  the {\technique} technique and I do not give details here.}  These
{\StateMachine} programs examine the state of the program and, based
on that, produce a prediction about the program's ultimate behaviour,
which is either \textsc{Crash}, \textsc{Survive}, or
\textsc{Unreached}.  \textsc{Crash} and \textsc{Survive} indicate that
the program either will or will not, respectively, reproduce the bug
which is under investigation.  \textsc{Unreached} indicates that a
particular path through the {\StateMachine} should be ignored for some
reason, most often because the matching path through the original
program would suffer some fatal error other than the bug of interest.
...
$\mathsf{Entry}_t{A_i}$ & True if thread $t$ starts with instruction $A_i$. \\
...
straightforward,\kern-.6pt\fnote{My implementation uses
...
More importantly, it does not lead to an effective analysis.  Many
library functions have simple interfaces but complex implementations;
...
least, would require a more powerful symbolic execution than
...
near to the buggy code without influencing its behaviour.
...
\item \emph{Copy propagation}, based on the algorithm used in the
...
any possible SAVs being omitted.  It is therefore always safe to do
so.
...
{\technique} is to first generate a set of minimal \glspl{cfg}
...
\shortrightarrow B_0$.  This will cause the $C_0 \shortrightarrow A_0$
and $B_1 \hspace{-.5pt} \shortrightarrow B_0$ edges to be broken, as
shown:\vfill
\noindent
...
{\AStateMachine}, simplified, and passed to the rest of the analysis.
...
The symbolic execution engine used by {\technique} is, for the most
part, quite conventional.  I give only a brief overview of the most
...
\item The {\StateMachine} speicher is represented by a sequence of
  ...
  This is not a particularly efficient approach.  {\Technique}
  ...
  {\StateMachine}.  Second, {\technique} maintains a cache of
...
\item Aliasing problems are resolved lazily.  This means that if the
  ...
  selects an appropriate result.  {\Technique} only determines which
  precise \state{Store} operation should provide the loaded data when
  that data is needed.  This is often faster then resolving these
  queries eagerly.\kern.7pt\fnote{Note that this is not quite the same
    ...
    state are represented in the abstract state.  {\Technique} uses a
    fixed abstraction; the laziness here lies in the \emph{way} the
    abstract state is represented, not in \emph{what} is represented.}
...
\item The engine does not use any kind of incremental abstraction
  ...
  that CEGAR provides far less benefit.  The use of lazy aliasing
...
\item Unlike most symbolic execution engines, the one used by
  {\technique} does not attempt to detect when it revisits a
...
{\technique}'s performance.  Nevertheless, even this relatively simple
system is sufficient to generate some results in a useful selection of
cases.
...
\begin{sanefig}
  ...
  \caption{A basic {\StateMachine} cross-product algorithm, expressed
    as a node replacement graph grammar.  $m$ matches boolean
    expressions; $A_0$, $A_1$, and $A$ match fragments of the crashing
    {\StateMachine}; $B_0$, $B_1$, and $B$ match fragments of the
    interfering {\StateMachine}; $a$ and $b$ match individual states
    within the crashing and interfering {\StateMachines},
...
engine: {\technique} already needs all of the simplifiers in
...
\item
  ...
  survive in isolation, and because I-atomic is implied by
  \gls{ic-atomic}.  \gls{c-atomic} is independent of the interfering
      {\StateMachine}, and so only needs to be calculated once for
      every crashing {\StateMachine} rather than once for every pair
      of crashing and interfering {\StateMachines}.
\end{itemize}
...
The stack model is built using a conventional static function-local
pointer escape analysis~\cite[pages 140--141]{Appel2004} based on the
...
non-stack locations, implemented as a Valgrind
skin~\cite{Nethercote2007}.  This analysis works by dividing memory
into fixed-size chunks\footnote{For the prototype implementation,
  these chunks are eight bytes, matching the platform word size. This
  ...
  larger ones.} and then tracking which instructions access each
chunk.  If two instructions are ever observed to access the same chunk
without an intervening call to a function such as \texttt{free} then
they are marked as potentially aliasing; otherwise, they are marked as
...
{\Technique} includes one minor refinement to the basic dynamic
...
{\Technique} makes several important assumptions in order to simplify
implementation, and some of these can impact the correctness of the
analysis.  I now briefly discuss these assumptions and their effects,
and possible ways of removing them.

\subsection{Incomplete library models}
As has already been discussed, {\technique} relies on hand-written
{\StateMachine} implementations of system library functions.  If no
such implementation is available then {\technique} uses a stub
implementation which simply sets the return address register to a new
free variable.  While this approach does have some advantages, most
notably in being able to incorporate useful higher-level information
on library interfaces without needing to analyse complex library
implementations and in being able to perform analysis without access
to a library implementation, it carries obvious risks of incorrectly
characterising bugs.  This is particularly regrettable since the
{\technique} technique itself is just as applicable to library code as
it is to code in the main program.  This weakness could be eliminated
by arranging to load the library implementation, when available, into
the analysis tool along with the program itself, and then extending
the \gls{static cfg} building algorithms to trace across the
...
In addition to the library limitation, {\technique} simplifies the
processor's memory model in three ways.  First, the aliasing
...
{\technique}'s scalability and so this would be a poor trade-off.

Second, {\technique} assumes that every address in the program's
...
of the program.  {\Technique} will therefore not be able to model bugs
which depend on read-only memory or those where the
...
Third, {\technique} assumes that the program does not make use of any
run-time generated code or run-time code modifications.  This would be
a difficult limitation to remove.  The \glspl{dynamic cfg} and
{\StateMachines} are generated from the program binary's \gls{static
  cfg}, and this will not accurately reflect the program's actual
instructions if those instructions can change while the program is
running.  Solving this problem is impossible in the general
...
{\technique}, although the rarity of run-time code generation suggests
that this would be a dubious use of engineering effort.  Even with
this extension {\technique} would not be able to support true
self-modifying code, as the {\StateMachines} cannot be modified while
they are being symbolically executed; removing \emph{that} limitation
is probably impractical while retaining the {\StateMachine}
...
The final simplification made by {\technique} is in its concurrency
model.  The only form of concurrency supported by {\technique} is
simple multi-thread concurrency within a single program.  It does not,
for instance, consider races with signal handlers, or inter-process
races mediated via shared memory regions or \texttt{ptrace}-like APIs,
or races with hardware DMA.  While these mechanisms are much less
...
analysis might need some modest extensions to determine when to do so.
Supporting inter-process concurrency would require more fundamental
changes to the {\technique} tool, as it would need to be able to model
multiple address spaces at the same time, but would require few
fundamental changes to the {\technique} technique itself.
