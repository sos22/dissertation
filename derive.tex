...
One problem remains to be solved: how is a thread to determine with
which thread it is supposed to be communicating?  In the example,
there might be many program threads running the \texttt{crashing} and
...
hypothetical threads so as to make the ordering
consistent\editorial{Feels like I want to say more there.}.
...
\begin{figure}
  ...
  \begin{tikzpicture}
    ...
    \node (n15) [below left = of n24b] {Advance to \textbf{Emul}};
    \node (n35) [below right = of n24b] {Advance to \textbf{Succ}};
    \draw[->] (n1m) -- (n10);
    \draw[->] (n10) -- (n11);
    \draw[->] (n11) -- (n22);
    \draw[->] (n3m) -- (n30);
    ...
    \draw[->,style={dashed}] (n11) to [bend right=30] (n15);
    \draw[->,style={dashed}] (n31) to [bend left=30] (n35);
  \end{tikzpicture}
  }
