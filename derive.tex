...
diagrams, or BDDs\needCite{}, which are themselves expressed in terms
of the expression language.  These BDDs allow much more efficient
implementations of many common operations than would be possible with
a scheme based entirely on unconstrained
...
considered by using the \backref{program model}.
...
\backref{interfering store} cannot possible influence the behaviour of
...
either one of the \backref{interfering stores} or a load operation
which might alias with one of the stores in the \backref{crashing
  thread} {\StateMachine}.  Instructions prior to the first
...
a set of minimal \backref{CFGs} (i.e. ones in which every root is a
...
\todo{Shrink a bit.} The core of the approach is to take a pair of {\StateMachines}, one
...
more often than \texttt{l2}\editorial{It's a bit of an abuse to talk
  about the frequency of a non-periodic process, but I think it's
  fairly clear what I mean.}.  Alternatively, the program could be
...
\todo{Should arguably try to show that it's a good factorisation, in
  ...
  a pretty hideous worst-case complexity.}
...
program reaches the starting point of any of the CFGs involved in one
...
\item
  ...
  operation\footnote{{\Technique} has some knowledge of simple
    ...
    programmer-implemented synchronisation operations \todo{Cite one
      of the papers on inferring program-specific sync structures.},
...
search problem\editorial{cite a textbook maybe?}.  The algorithm
