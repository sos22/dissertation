...
with source code and hence (mostly\editorial{But not always; dig up
  some references.}) assume that such information is available.  This
...
\begin{landscape}
\begin{longtable}{lllp{4.5cm}p{10.5cm}}
...
\newpage
 ...
 &                              & \state{tmp}  & {\STateMachine} temporary \\
 & \state{StartAtomic}          &              &                 & \multirow{3}{10.5cm}{Mark the start and end of atomic blocks, used to constrain the set of schedules which must be considered; see Section~\ref{sect:using:build_cross_product}.} \\
 & \state{EndAtomic}            \\
\\
...
\caption{Types of {\StateMachine} states.  \todo{StartFunction, EndFunction, StackLayout, alias set on ImportRegister? Also, second page looks a bit silly}}
...
general expressions is impossible\editorial{Cite the incompleteness
  theorem.}, and so while ordinary BDDs are canonical representations
...
before that instruction in a single thread\editorial{ignoring signal
  handlers}.
...
\item
  ...
  $D_0$ but the \backref{CFG} does not allow that to happen.  These
  are converted to branches to the special \state{Unreached} state,
...
\item
  ...
  successors in the unrolled \backref{CFG}.  Each of the $B_i$
  \backref{CFG} nodes will be represented by a separate
          ...
          program's path through the unrolled \backref{CFG}.  See, for
  ...
  the unrolled \backref{CFG} rather than control flow within the
...
{\StateMachine} can then be converted to \backref{SSA} form and passed
to the rest of the analysis.

\begin{figure}
\begin{tikzpicture}
  \node[stateIf,initial above] (l1) {\state{If} $\entryExpr{\mai{threadId}{A_0}}$};
  \node[stateSideEffect,below = of l1] (l2) {$A_0$: \state{Copy} $\mathit{rdx}$ to $\mathit{rcx}$};
  \node[stateIf,below = of l2] (l3) {$A_0'$: \state{If} $\controlEdge{threadId}{A_0}{B_0}$};
  \node[stateSideEffect,below = of l3] (l4) {$B_0$: \state{Load} $\mathit{rcx}$ to $\mathit{rcx}$};
  \node[stateSideEffect,below = of l4] (l5) {$C_0$: \state{Load} $\mathit{rcx}+8$ to $\mathit{tmp}$};
  \node[stateIf,below = of l5] (l6) {\state{If} $\mathit{tmp} = 0$};
  \node[stateIf,below = of l6] (l7) {$D_0$: \state{If} $\mathit{BadPtr}(\mathit{rcx})$};
  \node[stateSideEffect,below right = of l3] (l8) {$B_1$: \state{Load} $\mathit{rcx}$ to $\mathit{rcx}$};
  \node[stateSideEffect,below = of l8] (l9) {$C_1$: \state{Load} $\mathit{rcx}+8$ to $\mathit{tmp}$};
  \node[stateIf,below = of l9] (l10) {\state{If} $\mathit{tmp} = 0$};
  \node[stateSideEffect,below right = of l1] (l11) {$C_2$: \state{Load} $\mathit{rcx}+8$ to $\mathit{tmp}$};
  \node[stateIf,right = of l3] (l12) {\state{If} $\mathit{tmp} = 0$};
...
this list are referred to as the \introduction{interfering stores} for
...
\backref{communicating instruction}, end with an \backref{interfering
  store}, and contain at most \backref{$\alpha$} instructions.  This
...
\begin{tikzpicture}
  ...
  \draw node [right=of box1] {
    \begin{tabular}{lccccc}
             & \multicolumn{2}{c}{$\mathit{min\_to}$} & \multicolumn{2}{c}{$\mathit{min\_from}$} & overall min\\
             & $A$ & $C$ & $A$ & $C$ \\
      $A$    & 0   & 2   & 0   & 1 & 0\\
      $B$    & 2   & 1   & 1   & 2 & 2\\
      $C$    & 1   & 0   & 2   & 0 & 0\\
...
\begin{tikzpicture}
  ...
  \draw node [right=of box1] {
    \begin{tabular}{lccccc}
      labels & \multicolumn{2}{l}{$\mathit{min\_to}$} & \multicolumn{2}{l}{$\mathit{min\_from}$} & overall min\\
            & $A$ & $C$ & $A$ & $C$\\
      $A$   & 0 & 2 & 0 & $\infty$ & 0\\
      $A_1$ & 0 & 2 & 3 & 1 & 1\\
      $B$   & 2 & 1 & 1 & 2 & 2\\
...
matching \backref{$\alpha$}, and so there can be no paths through the
...
\begin{tikzpicture}
  ...
  \node (A2) [DupeCommCfgInstr, left=of C1] {$A_2$};
  \node (C2) [DupeInterferingCfgInstr, below=of B4] {$C_2$};
  \node (C3) [DupeInterferingCfgInstr, right=of A1] {$C_3$};
  \draw[->] (A) -- (B);
  \draw[->] (A1) -- (B3);
  \draw[->] (B) -- (C);
  \draw[->] (B) -- (B1);
  ...
  \draw node [right=of box1] {
    \begin{tabular}{lccccc}
      ...
      $B$   & 2 & 1 & 1 & $\infty$ & 2\\
      $B_1$ & 2 & 1 & 2 & $\infty$ & 3\\
      $B_2$ & $\infty$ & 1 & 3 & $\infty$ & 4\\
      $B_3$ & 2 & 1 & 4 & 2        & 3\\
      $B_4$ & $\infty$ & 1 & 5 & 3        & 4\\
      $C$   & 1 & 0 & 2 & 0        & 0\\
      $C_1$ & 1 & 0 & 5 & 3        & 4\\
      $C_2$ & $\infty$ & 0 & 6 & 4        & 4\\
      $C_3$ & $\infty$ & 0 & 4 & $\infty$ & 4\\
    \end{tabular}
  };
\end{tikzpicture}
...
properly\footnote{Of course, this means that stack references near the
  ...
  different function contexts.  \todo{True for crashing threads;
    interfering ones are a bit more complex.}}.  This is not
...
analysis is complete and no part of the analysis times out \todo{I
  kind of contradict this a bit later.}.  It is not, however, sound,
...
generates an \introduction{inferred assumption}\editorial{I should
  probably be citing something for that}: a condition which expresses
...
\backref{inferred assumption} and the \backref{verification condition}
...
support an \backref{analysis window} larger than a few hundred
...
\item In addition to the obvious state such as the contents of memory
  ...
  which variables have been assigned to\editorial{Already said that.}.
  ...
  \todo{That's a moronic way of implementing this.  Should just use
    ...
    them.  Or I could just not talk about it all, since it doesn't
    really matter.}
...
{\StateMachine} survives, and \introduction{IC atomic}\editorial{Not
  convinced I use the sub-components of the inferred assumption often
  enough to need to name them.}, which gives the conditions under
...
in the \state{Survive} state\editorial{i.e. taking all paths which
  definitely survive, and excluding those which crash and which reach
  \state{Unreached}.}.  The inferred assumption itself is then formed
...
{\StateMachine} until it reaches the \state{Survive} state and then
...
{\StateMachines} in turn.  \todo{I can probably find a cite for that
  if I really try.}
...
\item There will be many pairs of states in the two input
  ...
  possible.
\item The \backref{cross-product machine} is only used to investigate
  ...
  considered in the \backref{inferred assumption}.  The cross-product
...
\item One of the successors of the initial configuration is (\{$A$,
  ...
  instantiates this output state as \state{Unreached}.  The later
  ...
  an access the output state should be \state{Unreached}.
...
\begin{figure}
  \begin{subfloat}
    \begin{tikzpicture}
      \node[stateSideEffect,initial above] (lA) {$A$: \state{Load} $x \rightarrow tmp$ };
      \node[stateIf,below = of lA] (lB) {$B$: \state{If} $tmp = 0$ };
      \node[stateSideEffect,below = of lB] (lC) {$C$: \state{Load} $x \rightarrow tmp'$ };
      \node[stateIf,below = of lC] (lD) {$D$: \state{If} $BadPtr(tmp')$ };
      \node[stateTerminal,below = of lD] (lH) {$H$: \state{Crash} };
      \node[stateTerminal,right = of lC] (lG) {$G$: \state{Survive} };
      \draw[->] (lA) -- (lB);
      \draw[->,ifTrue] (lB) -- (lG);
      \draw[->,ifFalse] (lB) -- (lC);
      \draw[->] (lC) -- (lD);
  ...
  \begin{subfloat}
    \begin{tikzpicture}
      \node[stateIf,initial above] (lE) {$E$: \state{If} $y \not= 0$};
      \node[stateSideEffect,below right = of lE] (lF) {$F$: \state{Store} $0 \rightarrow x$};
      \node[stateTerminal,below left = of lF] (lI) {$I$: \state{Survive} };
      \draw[->,ifTrue] (lE) -- (lI);
      \draw[->,ifFalse] (lE) -- (lF);
      \draw[->] (lF) -- (lI);
    \end{tikzpicture}
...
\begin{figure}
  \begin{tikzpicture}[align=center]
    ...
    \node[stateIf, below = of D] (F) {(\{$B$, {\true}\}, \{$E$, {\false}\})\\\state{If} $tmp = 0$ };
    \node[stateTerminal, below = of E] (G) {(\{$A$, {\false}\}, \{$I$, {\true}\})\\\state{Unreached} };
    \node[stateTerminal, right = of F] (H) {(\{$G$, {\true}\}, \{$E$, {\false}\})\\\state{Unreached} };
    \node[stateIf, below = of F] (I) {(\{$C$, {\true}\}, \{$E$, {\false}\})\\\state{If} $\happensBefore{C}{F}$ };
    ...
    \node[stateIf, below = of J] (L) {(\{D, {\true}\}, \{$E$, {\false}\})\\\state{If} $BadPtr(tmp')$ };
    \node[stateSideEffect, right = of K] (M) {(\{$C$, {\true}\}, \{$I$, {\true}\})\\\state{Load} $x \rightarrow tmp'$ };
    \node[stateTerminal, below = of L] (N) {(\{$H$, {\true}\}, \{$E$, {\false}\})\\\state{Unreached} };
    \node[stateTerminal, right = of N] (O) {(\{$G$, {\true}\}, \{$E$, {\false}\})\\\state{Unreached} };
    \node[stateIf, below = of M] (P) {(\{D, {\true}\}, \{$I$, {\true}\})\\\state{If} $BadPtr(tmp')$ };
    \node[stateTerminal, left = of P] (Q) {(\{$G$, {\true}\}, \{$I$, {\true}\})\\\state{Survive} };
    \node[stateTerminal, below = of P] (R) {(\{$G$, {\true}\}, \{$I$, {\true}\})\\\state{Crash} };
    \draw[->,ifTrue] (A) -- (B);
    \draw[->,ifFalse] (A) -- (C);
    \draw[->,ifTrue] (C) -- (D);
    \draw[->,ifFalse] (C) -- (E);
...
\todo{Kind of already said this.} Importantly, the cross-product
     ...
     that the program will reach a \state{Crash} state precisely when
...
\todo{This uses lots of words to say something fairly obvious.}
...
\begin{figure}
  \begin{tikzpicture}
    ...
    \draw[<->, dashed] (L2) -- (S1);
  \end{tikzpicture}
  \caption{An example aliasing and control flow graph. \todo{Ugly diagram}}
...
\texttt{l2} runs\editorial{It's a bit of an abuse to talk about the
  frequency of a non-periodic process, but I think it's fairly clear
  what I mean.}.  Alternatively, the program could be modified such
...
the side-condition: if the message operation place represents the
...
$D_0$ and $D_1$ CFG nodes in the first thread represent duplicates of
the same program instruction created by the loop unrolling algorithm
(see Section~\ref{sect:derive:handling_loops}).  These CFG nodes are
...
\begin{figure}
  \begin{tikzpicture}
    ...
    \node[below = of C] (D) {$D_0$};
    \path (node cs:name=D,anchor=north west) node [below left] {\shortstack[r]{$\textsc{rax}_1$, $\textsc{rcx}_1$\\$\textsc{rbx}_2$, $\textsc{rcx}_2$,\\$\controlEdge{1}{C}{D_0}$}};
    \node[left = 4cm of D] (D1) {$D_1$};
    \node at (t2 |- A) (X) {X};
  ...
  \end{tikzpicture}
  \caption{Figure~\ref{fig:place_conditions_example} extended to
    show sub-expression availability.}
...
become available in the program CFG.  In the example, the first
  ...
  \centerline{
    \begin{tabular}{ccccc}
      ...
      \end{tikzpicture}} & \raisebox{30mm}{$\bigwedge$} & \raisebox{16mm}{\begin{tikzpicture}
        ...
        \draw[BddFalse] (b) -- (false);
      \end{tikzpicture}} \\
      (a) Reordered BDD & \multicolumn{3}{c}{(b) Condition evaluatable at $A$} & (c) Residual condition
    \end{tabular}
  }
...
shows how this is done for CFG node $A$.  The available inputs are
...
which is evaluatable at CFG node $A$, (b), and a residual which must
...
of the bugs which is to be enforced\editorial{Orphan sentence?}.
...
\item
  ...
  operation\footnote{{\Technique} has some knowledge of simple
    ...
    programmer-implemented synchronisation operations \todo{Cite
...
This is most easily understood as a kind of search
problem\editorial{cite a textbook maybe?}.  The algorithm starts in a
...
\todo{I'm pretty certain I've already said this.} The interpreter has
...
As an example of the former, consider the threads illustrated in
...
by this kind of instruction-level fix\footnote{But see future work
  Section~\ref{sect:fw:better_fixes} for a possible alternative scheme
  which would make use of it.}, so including it in the fix is
...
enforcer\editorial{ref?}, these regions are defined in terms of the
...
The solution to these problems is similar to that used by enforcers:
