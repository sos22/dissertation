...
\item Identify all of the instructions which the \gls{crashingthread}
  ...
  as a \glslink{dynamic cfg}{dynamic Control Flow Graph (CFG)}; the
...
bugs, in which case the \gls{crashing instruction} is initially
...
straightforward,\kern-.6pt\fnote{{\Implementation} uses LibVEX~\cite[Chapter
...
influencing its behaviour.\kern-.2pt\fnote{Even with a more complete
...
call\kern.2pt.\kern-.3pt\fnote{Note that this includes other calls to \texttt{free} in
...
another sequence of instructions.  Any \glspl{sav} which could be
...
any possible \glspl{sav} being omitted.  It is therefore always safe to
do so.
...
\item Aliasing problems are resolved lazily.  This means that if the
  ...
  resolving these queries eagerly.\kern.7pt\fnote{Note that this is not quite
...
operation in the {\StateMachine} language,\kern.3pt\fnote{In
  particular, the number of steps which such a cross-product emulating
  {\StateMachine} could take would be statically bounded, and so the
  number of states of the input {\StateMachines} which it could
  consider would also be statically bounded, and so there is no single
  {\StateMachine} which can compute the cross-product of two other
  arbitrary {\StateMachines}.} and so it must be split between a
...
it must be able to tell when the type of structure stored at a
...
processors,\kern-.1pt\fnote{Indeed, many processors do not support
  ...
  operating system, and performance in that case is generally abysmal
...
case,\kern-.1pt\fnote{Consider a program P in a model of computation C which
