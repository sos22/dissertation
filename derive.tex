...
\item Identify all of the instructions which the \gls{crashingthread}
  ...
  as a dynamic \gls{cfg}; the details are given in
...
%% \todo{This is really boring, but it's useful to be able to refer back
%%   to it.}  Before attempting to analyse any specific bug, {\technique}
%% first attempts to recover the program's control flow structure,
%% expressed as a control flow graph (CFG) over its static instructions.
%% Building this \gls{cfg} is generally straightforward, by simply
%% starting at the program's first instruction (found from the ELF
%% metadata) and working forwards from there, with two main exceptions:
%% branches from library code back into the main program, and computed
%% branches within the program.  These are difficult to handle because
%% their targets are not apparent when analysing the program's static
%% text.  {\Technique} relies on a dynamic analysis which records all
%% such branches in some small number of training runs, and blithely
%% assumes that the behaviour observed during training is the only
%% behaviour possible.  {\Technique} will produce an incomplete set of
%% \glspl{verificationcondition} if this assumption is false.  This
%% weakness is more aesthetic than meaningful:
%% \glspl{verificationcondition} are primarily when building
%% \glspl{bugenforcer}, and enforcers are themselves a far more important
%% source of incompleteness than the static \gls{cfg}, and so minor
%% lacunae in the \gls{cfg} cause few real problems.
...
\todo{Not brilliantly written.} The initial \gls{static cfg} is
generated by taking the program's complete \gls{cfg} and trimming it
down to contain only those instructions which can reach
the crashing
instruction within \gls{alpha} instructions.  This strategy depends on
first being able to generate the complete \gls{static cfg} in the
first place.  {\Technique} uses a standard recursive traversal
disassembler~\cite{Schwarz} to do so.  Information collected during
the initial dynamic analysis phase is used to identify the \gls{cfg}
roots, where program code is invoked by system libraries, and to find
the successor instructions of indirect branch instructions.  This
information could potentially be incomplete, leading to an incomplete
\gls{cfg}, and if that happens then {\technique} will produce an
incomplete set of \glspl{verificationcondition}.  This weakness is
more aesthetic than meaningful: \glspl{verificationcondition} are used
primarily to build \glspl{bugenforcer}, and enforcers are themselves
inherently incomplete, and so minor lacunae in the \gls{cfg} cause few
real problems.
...
\begin{sanefig}
  ...
  \caption{Small-step semantics for single-thread {\StateMachines};
    ...
    variables, and $m$ the \gls{speicher} under the environment $e$,
...
\end{tikzpicture}
\caption{{\STateMachine} generated from the \gls{dynamic cfg} shown in
...
\item \emph{Copy propagation}, based on the algorithm from the
  ...
  {\technique} can make use of \state{Assert} side-effects during this
...
\begin{tikzpicture}
  \node (A) at (0,2) [CommCfgInstr] {$A_0$};
  \node (B) [CfgInstr, below=of A] {$B_0$} edge [in=210,out=150,loop,killEdge] ();
...
the two length four paths $A_0 \shortrightarrow B_0 \shortrightarrow
B_1 \hspace{-.5pt} \shortrightarrow B_0$ and $A_0 \shortrightarrow B_0
\shortrightarrow \hspace{1pt} C_0 \shortrightarrow A_0$.  This will
cause the $B_1 \hspace{-.5pt} \shortrightarrow B_0$ and $C_0
\shortrightarrow A_0$ edges to be broken, as shown:\\
\noindent
...
is eliminated by duplicating $B_0$: \\
\noindent
\centerline{
\inlinebox{
\begin{tikzpicture}
...
\item The {\StateMachine} speicher is represented by a sequence of
  ...
  \state{Store} operations are implemented by simply adding them to
  the update list and \state{Load} operations have to scan back
  through the list to find a matching \state{Store}.
...
the interfering one\footnote{There is a loose but perhaps helpful
  ...
  the state reachability graphs maintained by most symbolic execution
  ...
  \todo{Not sure how much that helps, but I thought it was
    interesting.}}\!\hspace{-.1em}\!.
...
variant of the techniques described by Cozzie et
al.~\cite{Cozzie2008}, but I have not investigated that at this
time\editorial{Move to related work?}.
