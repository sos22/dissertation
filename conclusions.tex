...
\subsection{Finding and characterising bugs}
...
format such as LLVM bitcode\cite{Lattner2013}.  These provide the
...
The analysis could also be extended to cover a wider class of
concurrency error.  At present, it can be used only to investigate
atomicity violation bugs, defined to be those in which running some
fragments of a program atomically does not crash but interleaving them
does.  There is a dual algorithm which finds fragments of the program
such that running them atomically leads to a crash but running them in
parallel potentially avoids it, and this algorithm might prove
effective at identifying order violation bugs.  The main barrier to
pursuing this idea is the limited size of {\technique}'s
\gls{analysiswindow}, as ordering violation bugs generally involve
larger fragments of code than atomicity violation ones.
...
\todo{Need to check this one over a little.}
