...
\usepackage{tikz}
\usepackage{tabularx}
\usetikzlibrary{arrows,decorations.pathmorphing,backgrounds,positioning,fit,petri,automata,shapes}
...
\item
  W isolation \editorial{God only knows what I'm going to do as an argument here.}.
\item
...
\item
  The concurrent constraint is true by construction\editorial{Do I believe this?}.
\end{itemize}
...
\item
  W isolation \editorial{Don't know what to say about this either}.
\item
...
\item
  The concurrent constraint is true by construction\editorial{Do I believe that?}.
\end{itemize}

We can then use an \editorial{kind of induction, but not really sound} induction argument to suggest that $N_r$ can be increased without hiding bugs.
...
\item
  ...
  The size of the necessary difference depends on the exact details of the ISA and operating system\editorial{This is really begging for some quick experiments}.
\end{itemize}
...
\editorial{But it doesn't do so quite yet...}
...
\verb|l9| can therefore be delayed until it receives a message which allows the verification condition to succeed\editorial{Not actually how this is implemented right now; need to fix that.}.
...
\draw[->] (RecvMsg) -- (OrigInstr) -- (VerfCond) -- (SendMsg) -- (FindSucc);
\draw[->,dashed] (FindSucc.west) to [bend left=90] (RecvMsg.west);
\draw[->] (RecvMsg) -- (RecvdMsg) -- (OrigInstr);
\draw[->] (RecvMsg) -- (Exit);
\draw[->] (VerfCond) -- (CondFail) -- (Exit);
\draw[->] (FindSucc) -- (Exit);
\end{tikzpicture}
\caption{Intra-instruction state machine.  The dashed arc indicates that the thread advances to the next instruction during that transition.}
\label{fig:enforcement:intra_instruction_machine}
\end{figure}

The interpreter proceeds by advancing the instruction phase through the intra-instruction state machine, shown in figure~\ref{fig:enforcement:intra_instruction_machine}.
The meaning of the phases is as follows:
...
\end{verbatim}
    ...
    \verb|LOCK|ed instructions are instead replaced with a \verb|CMPXCHG| loop in the conventional way\editorial{Not sure I'll get away with that}.
  \item
...
The solution to this is to modify the interpreter by effectively taking the cross product over all of the state which is uncertain, in effect converting the simple interpreter into a high-level interpreter which interprets sets of simple interpreters in parallel\editorial{Kind of a cross product; also quite a lot like a power set construction}.
...
\todo{The way this is actually works is \emph{much} more complicated than that, but that's the important bit.}
...
The final intra-instruction state machine then looks like this:
...
\todo{And then we do all of that, but as a compiler rather than an interpreter, which doesn't make it any easier to understand.}
...
Also recovery from spurious segfaults in the patch due to e.g. LD operations.
